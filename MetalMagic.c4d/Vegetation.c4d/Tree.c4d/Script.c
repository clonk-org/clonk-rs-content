...
{
  // Bei aktivierter Baumregel, Anti-Brenn Effekt erzeugen
  if(ObjectCount(NBTR))
    AddEffect("NoBurn", this, 500, 0, this);
  // Bewegungsschwelle
...
{
  [$TxtChop$|Image=TRMS|Condition=IsStanding]
...
private func Seed()
{
  // Keine brennenden Bäume, dann wachsen auch keine.
  if(ObjectCount(NBTR)) return;
...
private func FxNoBurnEffect(string szNewEffectName, object pTarget) {
  // nicht anbrennen lassen
  if(WildcardMatch(szNewEffectName,"*Fire*")) {
    return -1;
  }
  // gleichen Effekt brauchen wir nicht nochmal
  if(szNewEffectName == "NoBurn") return -1;
}

private func FxNoBurnDamage(object pTarget, int iEffectNumber, int iDmgEngy, int iCause)
{
  // Wenn Bäume nicht brennen, sollten sie auch nicht durch Flints und Co. Schaden bekommen
  // denn sonst fallen sie irgendwann einfach um
  if(ObjectCount(NBTR) && iCause != 3) return 0;
  // ohne Regel oder bei Fällen normaler Schaden
  return iDmgEngy;
}
