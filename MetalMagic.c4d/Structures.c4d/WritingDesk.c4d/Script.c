...
public func ProductionMenu(pWorker) {
  ...
  //Nur für Mages
  if(GetID(pWorker) != MAGE)
  {
   PlayerMessage(GetOwner(pWorker),"$OnlyMages$",this);
   Sound("Error");
   return 0;
  }
...
private func ProductionCancel() {
  // Arbeit abbrechen
  if (GetAction() == "Write") SetAction("Idle");
  // Clonk(s) auswerfen
  var clnk;
...
public func CastleChange() {
  ...
  if(!FindObject(CS2B, 0, -1)) return 1;
