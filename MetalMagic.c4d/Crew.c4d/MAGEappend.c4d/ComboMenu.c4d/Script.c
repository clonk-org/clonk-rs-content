...
local bStaffSpells;  // Ein Stab wird verwendet
...
public func Create(object pCommandObject, object pMenuObj, id idSpellClass, bool faFixedDir)
  ...
  // Bei einem Stab, Flag setzen
  if(idSpellClass) if(idSpellClass->~IsStaff()) bStaffSpells = 1;
  // Symbole erstellen
  return CheckSpells();
  }
...
private func GetStaffSpell(iIndex)
{
  var idSpell;
  var i = 0, iCount = 0;
  while(idSpell=GetDefinition(i++,1))
  {
    if(idSpell->~GetSpellClass())
      if(iCount++ == iIndex) return idSpell;
  }
}

private func GetSpell(i)
{
  if(bStaffSpells) return(pTargetObject->HasStaff()->~GetSpell(i));//return(idSelectedElement->~GetSpell(i));
  else return GetPlrMagic(GetOwner(), 0, i);
}
...
private func CheckSpells()
  ...
  while (idSpell =GetSpell(i++))
  //while (idSpell = GetDefinition(i++, 1<<17))
    if ((idClass=idSpell->~GetSpellClass(this)) || bStaffSpells)
      if (idClass == idSelectedElement || idClass == MISC || !idSelectedElement || bStaffSpells)
        {
        // Tastenkombination für den Spruch abfragen
    szCombo = 0;
    if(bStaffSpells) szCombo = pTargetObject->HasStaff()->~GetSpellCombo(i-1);
    if(!szCombo && !idClass) continue;
        if(!szCombo) szCombo = GetSpellCombo(idSpell, idClass);
        // Spruch gefunden? (Keine weiteren Tasten erforderlich) -> Ausführen!
        if (szCombo == szCastKeys) return DoSpell(idSpell);
...
private func ClearSpellSymbols()
  {
  var pObj; while (pObj = FindObjectOwner(MGSM, GetOwner(), 0,0,0,0, 0, 0,this)) RemoveObject(pObj);
  }
...
protected func DoCastControl(szControl)
  ...
  if (szControl == "L") if (GetDir(pTargetObject) || fFixedDir) szControl="1"; else szControl="3";
  if (szControl == "R") if (GetDir(pTargetObject) || fFixedDir) szControl="3"; else szControl="1";
  
  // War das der erste Kombo-Click?
  if(!szCastKeys)
    {
    // Elementsound abspielen
    if(szControl == "5") Sound("MagicElementAir", 0, pCallbackObject, 100, GetOwner(pCallbackObject)); // Hoch -> Luft
    if(szControl == "2") Sound("MagicElementEarth", 0, pCallbackObject, 100, GetOwner(pCallbackObject)); // Runter -> Erde
    if(szControl == "4") Sound("MagicElementFire", 0, pCallbackObject, 100, GetOwner(pCallbackObject)); // Werfen -> Feuer
    if(szControl == "6") Sound("MagicElementWater", 0, pCallbackObject, 100, GetOwner(pCallbackObject)); // Graben -> Wasser
    }
  // Ansonsten...
  else
    {
