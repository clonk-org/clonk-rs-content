...
public func Create(pCommandObject)
  ...
  SetAction("Grow",pCommandObject);
  // Überwachungseffekt fürs Schließen an den Clonk
  AddEffect("IntPRMUCheck", pCommandObject, 1, 0, this);
  }
...
protected func ControlLeft()
  ...
  if (GetAction() == "Turn" && GetDir() == DIR_Right)
    {
    SetDir(DIR_Left);
...
protected func ControlRight()
  ...
  // Drehend: ggf. Umdrehen
  if (GetAction() == "Turn" && GetDir() == DIR_Left)
    {
    SetDir(DIR_Right);
    SetPhase(TurnDuration() - GetPhase());
...
protected func Update(fUpdCon)
  ...
  while (iIndex--)
    ...
    if (pSymbol = MnuItemSymbol(iIndex))
      ...
      /*if (fUpdCon || true)*/
      		 SetCon(iSymbolSize, pSymbol);
      // An neue Stelle schieben
      SetVertex(0, 0, -iX, pSymbol); SetVertex(0, 1, -iY, pSymbol);
      }
    else
      // Neu erzeugen
      if (MnuItemSymbol(iIndex) = pSymbol = CreateObject(MGM2/*MnuItemID(iIndex)*/, iX, iY, GetOwner()))
      if(pSymbol)
        {
        pSymbol->SetSymbolGfx(MnuItemID(iIndex),this, pCallbackObject);
        SetVisibility(VIS_Owner, pSymbol);
        ObjectSetAction(pSymbol, "Attach", this);
        // Größe setzen (VOR dem Verschieben der Vertices!)
        if (fUpdCon || true) SetCon(iSymbolSize, pSymbol);
        SetVertex(0, 0, -iX, pSymbol); SetVertex(0, 1, -iY, pSymbol);
        }
    }
...
private func TurnDuration() { return 9; }
...
public func SelectID(idSel) { var i=iElementCount; while (i--) if (MnuItemID(i)==idSel) { iSelection=i; return 1; } }

public func Show(pMenuObj)
  {
  // Nicht wenn leer...
  if (!iElementCount) { RemoveObject(); return 0; }
  // Felder setzen
  ...
  SetViewCursor(GetOwner(), pMenuObj); // Paladin als ViewCursor
  }

global func CreatePalaRingMenu(pCommandObject)
  {
  var pMenu = CreateObject(PRMU,0,0,GetOwner(pCommandObject));
...
protected func FxIntPRMUCheckStop(object pTarget, int iEffectNumber, int iReason, bool fTemp)
  {
  if (fTemp) return;
  Close(true);
