...
public func Set(int iAngle, int iWidth, int iDist, int iT, pDmg, object pAtt, int iReflect)
{
  // optional parameters
  if(!iWidth) iWidth = 3;
  if(!iDist) iDist = 300;
...
private func HitCheck() {
  ...
  {
  	if(pDmgCallback ->~ LaserStrike(pObj,c, GetController()))
  	{
  		SetLaser(0);
...
func CheckGround(int length)
{
	...
	newlaser->Set(rout, 2, iMaxDist-length, iTime, pDmgCallback, pAttach, reflections-1);
}
