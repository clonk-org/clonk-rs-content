...
global func Freeze(object pObject, object pFrom, int time, int dmg)
{
  ...
  AddEffect("Freeze", pObject, 111, 5, 0, FREZ, GetController(pFrom), time, dmg); 
}
...
global func FxFreezeStart(pTarget, iEffectNumber, iTemp, iFromController, time, dmg)
{
  ...
  EffectVar(4, pTarget, iEffectNumber)=dmg*1000;
  return(1);    
}
...
global func FxFreezeTimer(pTarget, iEffectNumber, iEffectTime)
{
  ...
  if(!Random(r)) {
   ...
   if(pFrom)
     pFrom->DoEnergy(-d,pTarget, true);
   else
     DoEnergy(-d, pTarget, true);
  }
  return FX_OK;
}

global func FxFreezeEffect(string szNewEffectName, object pTarget, int iEffectNumber, int iNewEffectNumber, iFrom, amount, dmg)
{
  // nur einmal einfrieren
  if(szNewEffectName eq "Freeze") 
  { 
  	if(amount > EffectVar(0, pTarget, iEffectNumber) && (EffectVar(4, pTarget, iEffectNumber) <= dmg))
  		EffectVar(2, pTarget, iEffectNumber)=iFrom;
  	EffectVar(0, pTarget, iEffectNumber)=Max(EffectVar(0, pTarget, iEffectNumber), amount); 
  	EffectVar(4, pTarget, iEffectNumber)=Max(EffectVar(4, pTarget, iEffectNumber), dmg); 
  	return(-1);
   }
