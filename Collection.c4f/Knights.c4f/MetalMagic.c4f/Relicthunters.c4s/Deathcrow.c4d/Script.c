...
protected func Activity()
{
  ...
  {
    var aHostile = Find_Hostile(GetOwner());
    if(GetOwner()==-1) aHostile = Find_Not(Find_Owner(-1));
    pTarget = FindObject2(Find_OCF(OCF_CrewMember), Find_NoContainer(), aHostile, Find_Distance(SearchRange()));
  }
...
public func Attack(object pAttack)
{
  ...
  var aHostile = Find_Hostile(GetOwner());
  if(GetOwner()==-1) aHostile = Find_Not(Find_Owner(-1));
  pAttack = FindObject2(Find_AtPoint(-14 + 28 * GetDir(), 0), Find_OCF(OCF_CrewMember), Find_NoContainer(), aHostile);
