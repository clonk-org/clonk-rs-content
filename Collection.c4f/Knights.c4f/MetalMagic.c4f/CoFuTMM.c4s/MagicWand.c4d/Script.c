...
{
  [$DescCast$|Image=MCMS|Condition=ReadyToMagic]
  // Nicht bereit
  if(!ReadyToMagic(pCaller)) return(0);
  // Clonk soll anhalten
    SetComDir(COMD_Stop(),pCaller);
  // Menü öffnen
  CreateMenu(MCMS,pCaller,0,3,"$Wand01$",GetMagicEnergy());
 
  for(var cnt=0;cnt<GetLength(spells);cnt++)
    { 
     AddMenuItem( "$Wand02$", "DoMagic", spells[cnt], pCaller);
...
protected func DoMagic(id idSpell)
{
  ...
  if (GetMagicEnergy()<Value(idSpell) )
    return( Message("$Wand03$",this()), Sound("Error") );
