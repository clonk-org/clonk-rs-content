...
protected func Initialize()
{
 ...
 CreateObject(_SPK,LandscapeWidth()/2+27,0,-1);
 CreateObject(_SPK,LandscapeWidth()/2+28,0,-1);
 CreateObject(_SPK,LandscapeWidth()/2+29,0,-1);

 // Lehm in der Burg
 var cpr2=FindObject(CPR2,296,207);
 if(!ContentsCount(LOAM, cpr2))
...
func Script0()
{
	 // the left anvil has issues
 	var anvil=FindObject2(Find_ID(ANVL), Sort_Distance());
 	anvil->SetPosition(anvil->GetX(), anvil->GetY()-30);
...
protected func RelaunchPlayer(int player)
{
 ...
 for (var i; i < 2; i++)
 {
  var knight = CreateObject(PLDN, 0,0, player);
