...
func Activate(pCaster,pRealcaster) {
  ...
  var iNum = AddEffect("AirblastNSpell",0,130,1,0,GetID(),0,GetX(pCaster),GetY(pCaster),pCaster);
...
public func ActivateAngle(object pCaller, int iAngle)
  ...
  // its magic!
  var iNum = AddEffect("AirblastNSpell",0,130,1,0,GetID(),Abs(iAngle)-90,GetX(pCaller),GetY(pCaller),pCaller);
  if (iNum <= 0) { return RemoveObject(); }
  Sound("MgWind*");
...
func FxAirblastNSpellTimer(object pTarget, int iNumber, int iTime) {
  ...
  while(pObj=FindObject(0,iX-iSize,iY-iSize,iSize*2,iSize*2,OCF_Collectible|OCF_Alive,0,0,NoContainer(),pObj))
    ...
    if(GetAlive(pObj))
      ...
      Fling(pObj,Sin(iAngle,iSpeed+20)/9,-Cos(iAngle,iSpeed+20)/9);
      }
    else
      SetSpeed(Sin(iAngle,iSpeed+20),-Cos(iAngle,iSpeed+20)/9);
    }
...
public func GetSpellCombo(pMage) { return "33"; } // (1: Backward; 2: Down; 3: Forward; 4: Throw; 5: Jump; 6: Dig)
