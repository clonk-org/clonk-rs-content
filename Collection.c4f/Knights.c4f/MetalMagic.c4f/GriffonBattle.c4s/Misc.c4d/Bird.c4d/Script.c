...
protected func RandItemID()
{
  var iItems = [BRED, BLTP, FARP, XARP, STFN, EFLN, TENP, SBLP, PBLP, COKI];
...
{
  // Nest bauen	
  if(!pNest || OnFire(pNest)) { Activity(); return SetAction("Fly"); }	  
  DoCon(10, pNest);
...
protected func CatchBlow(iLevel, pObject)
{
  ...
  CastParticles("BirdFeather", 10,30, 0,1-0, 10, 20, RGBa(128,128,255,0), RGBa(255,255,255,127));
  if(GetController(pObject)>=-1 && Contents())
  {
    var pObj = Contents();
    Exit(pObj);
    if(GetHiRank(GetController(pObject)))
      GetHiRank(GetController(pObject))->Collect(pObj);
