...
protected func Initialize(a,b,c,d,e,f,g,h,i,j)
  ...
  if (ObjectCount(EA69_RuleID) && GetOwner()<0 && !ai_iRange && GetID()!=BGNT) ScheduleCall(this, "SetAI", 50, 1);
...
/*  if (!Random(10)) DoMagicEnergy(ai_iMagicRegen * (1+g_EA69_iDifficulty));
  ...
  else if(!Random(3)) for (obj in aProjectiles)
    {
...
private func AI_Home()
  ...
  if(pSainthood && GetID()!=MAGE)
  {
  ai_iHomeX = GetX(pSainthood); ai_iHomeY = GetY(pSainthood);
  }
...
private func AI_ExecThrow()
  ...
  if(!pEnemy)
  { 
    pEnemy = FindObject2(Find_Distance(60), Find_Category(C4D_Structure));
    if(!pEnemy) return;
    if(pProjectile)
    {
    if(!pProjectile->~ExplodeSize())
      if(!(pProjectile = FindContents(FLNT)))
         if(!(pProjectile = FindContents(SFLN)))
           if(!(pProjectile = FindContents(EFLN)))
             if(!GetEffect("IntFlint", this))
             {
               CreateContents(SFLN, pProjectile);
               AddEffect("IntFlint", this, 1, 35*5, this);
             }}
    else
      if(!GetEffect("IntFlint", this))
             {
    ...
    if(!pProjectile->~ExplodeSize()) return;         
  }
  if(!pEnemy) return;
...
private func AI_ExecMagician()
  ...
  if (!Random(ai_iCastChance))
    if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
        if (AI_DoMagic(MICS)) return true;
...
private func AI_CanCast(id idSpell)
  ...
  if (idSpell)
    ...
    if (idSpell->~GetSpellClass()) if(idSpell->~GetSpellClass()->~IsStaff())
	    if(idSpell->~GetSpellClass()->~IsStaff() != HasStaff()) return;
    }
...
private func AI_FindNearestEnemy()
  ...
  var obj, pSainthood = FindObject2(Find_ID(SNHD), Find_Func("IsPlayerHome"));
  // Objekte prüfen:
  // Muss leben und im Freien sein, wenn die KI auch im Freien ist...
  while (obj = FindObject(0, 0,0,-1,-1, OCF_Alive, 0,0, cont, obj))
    if (ObjectDistance(obj) > ai_iRange) break;    // ...nicht zu weit weg sein...
	    ...
	    if(pSainthood && GetID()!=MAGE)
	    if(!AI_CanCast())
            if(ObjectDistance(obj, pSainthood)>50)
            if(ObjectDistance(pSainthood)<ObjectDistance(obj, pSainthood)) continue;
...
public func AI_Equip()
  {
  // Nur einmal
  if (Contents() || !GetAlive() || GetID()==_ZMB || GetID()==_SSR) return;
...
public func IsMagiclonk()
  {
  return (GetID() == MCLK || GetID() == SCLK || GetID() == MAGE || GetID() == KING || GetID() == _SSR);
  }
...
global func Hostile(int iPlr1, int iPlr2)
  ...
  if (GetID() == ELEC || GetID() == BRKD) return inherited(iPlr1, iPlr2);
