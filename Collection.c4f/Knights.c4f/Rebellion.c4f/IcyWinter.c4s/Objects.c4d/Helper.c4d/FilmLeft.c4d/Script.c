...
public func StartFilm()
{
  ...
  for(var i=0,p,c; i<GetPlayerCount(); i++)
    if(Global(p=GetPlayerByIndex(i)) == Position_Left)
    {
      if(iPlr==-1) iPlr=p;
  ...
  // Clonks erzeugen
  pClonk1=CreateObject(KNIG,AbsX(66),AbsY(1144),iPlr); pClonk1->SetDir(DIR_Right);
  pClonk2=CreateObject(CLNK,AbsX(43),AbsY(1140),iPlr2); pClonk2->SetDir(DIR_Right);
  pClonk3=CreateObject(CLNK,AbsX(18),AbsY(1139),iPlr3); pClonk3->SetDir(DIR_Right);
  ...
  // Lore und Armbrust
  pLorry=CreateObject(LORY,AbsX(44),AbsY(1137),iPlr);
  pCannon=CreateObject(XBOW,AbsX(18),AbsY(1133),iPlr);
  // Objekte in Lore
...
private func Film1()
{
  ...
  AddCommand(pClonk1, "MoveTo",, 614,1216);
  SetCommand(pClonk2, "Call", this(),,,,"Film2b");
  AddCommand(pClonk2, "MoveTo",, 652, 1219);
  AddCommand(pClonk2, "PushTo", pLorry, 594,1217);
  SetCommand(pClonk3, "Call", this(),,,,"Film2c");
  AddCommand(pClonk3, "PushTo", pCannon, 549,1218);
}
...
private func Film2b()
{
  var pFire = CreateConstruction(CPFR, AbsX(652), AbsY(1219+8), iPlr, 1, 1, 0);
  SetCommand(pClonk2, "Call", this(),,,, "Film3b");
  AddCommand(pClonk2, "Build", pFire);
//  SetCommand(pClonk2, "Construct",, 652, 1219,, CPFR);
}

private func Film2c()
{
  SetCommand(pClonk3, "Call", this(),,,, "Film2d");
  AddCommand(pClonk3, "MoveTo",, 462, 1219);
...
private func Film4()
{
  ...
  for(var i=0; i<5; i++) CreateContents(ARWP, pLorry);
  for(var i=0; i<4; i++) CreateContents(FARP, pLorry);
  for(var i=0; i<2; i++) CreateContents(XARP, pLorry);
  ...
  for(var i=0; i<2; i++) CreateContents(AXE1, pLorry);
  for(var i=0; i<5; i++) CreateContents(SHIE, pLorry);
  for(var i=0; i<2; i++) CreateContents(BOW1, pLorry);
