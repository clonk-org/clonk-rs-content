...
//  Log("val=%d / ramp=%d", iLightness, iGammaRamp);
}
...
global func Fade(int iActualValue, int iFadeTo, int iGammaRamp, int iTimerInterval, int iFadeSteps)
{
  ...
  // Effekt starten
  if(!AddEffect("GammaFader_fx6C", 0, 300, iTimerInterval, 0,0, iActualValue, iFadeTo, iGammaRamp, iFadeSteps))
    return(0);
  // Fadeeffekt erfolgreich erzeugt
  return(1);
}

global func FxGammaFader_fx6CStart(object pTarget, int iEffectNumber, bool fTemp, var1, var2, var3, var4)
{
//  Log("Fader-Start: temp=%d ramp=%d", fTemp, var3);
  if(fTemp) return();
  // iStartValue
...
global func FxGammaFader_fx6CTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
...
global func FxGammaFader_fx6CStop(object pTarget, int iEffectNumber, int iReason, bool fTemp)
{
//  Log("Fader-Stop: temp=%d", fTemp);
  if(fTemp) return();
  var iFadeTo      = EffectVar(1, pTarget, iEffectNumber);  
  var iGammaRamp   = EffectVar(2, pTarget, iEffectNumber);
  SetFadeLightness(iFadeTo, iGammaRamp);
}

/*
global func FxGammaFader_fx6CEffect(string szNewEffectName, object pTarget, int iEffectNumber, int iNewEffectNumber, var1, var2, var3, var4)
{
  // Bei neuen Fade-Effekten auf selber Gamma-Rampe den alten Effekt löschen
  if(szNewEffectName eq "GammaFader_fx6C" && EffectVar(2, pTarget, iEffectNumber) == var3)
