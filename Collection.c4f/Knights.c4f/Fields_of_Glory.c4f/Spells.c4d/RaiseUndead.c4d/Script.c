...
protected func FxUndeadSpellStart(object pTarget, int iEffectNumber, int iTemp, int iController) {
  ...
  // Laufgeschwindigkeit, Schwimmgeschwindigkeit und Sprunghöhe und hangeln und klettern werden reduziert
  ...
  DoPhysical("Scale",-20000,PHYS_Temporary,pTarget);
  DoPhysical("Hangle",-20000,PHYS_Temporary,pTarget);
...
protected func FxUndeadSpellTimer(object pTarget, int iEffectNumber, int iTime) {
  ...
  // Tote Clonks machen keinen Timercall
  if(!GetAlive(pTarget)) return();
  ...
  //Nach Tränken suchen und austrinken
  for (var pPotion in FindObjects(Find_Container(pTarget),Find_Func("IsPotion") ) ) pPotion->Activate(pTarget);

  // Bin ich ein Ritter oder ein König? Dann Ritterbefehle verwenden (letzte Reste des Kriegers leben noch).
  if ( GetID(pTarget) == KNIG || GetID(pTarget) == KING )
  {
	// Bewaffnet sich gerade?
	if (pTarget->GetOrder() == pTarget->OrderArm())
		if (pEnemy && ObjectDistance(pTarget, pEnemy) < 400) return(pTarget->SetOrderAttack(pEnemy));
	// Greift gerade an?
	if (pTarget->GetOrder() == pTarget->OrderAttack())
	{
		if (!pEnemy) return(pTarget->SetOrderNone());
		if ( ObjectDistance(pTarget->GetData(), pEnemy) > 50 ) return(pTarget->SetOrderAttack(pEnemy));
		if (!pTarget->IsFullyArmed() && ObjectDistance(pTarget, pEnemy) > 400) return(pTarget->SetOrderArm());
	}
	// Tut grade nix?
	if (!pTarget->GetOrder())
	{
		// Feind gefunden und bewaffnet?
		if (pEnemy && pTarget->IsFullyArmed()) return(pTarget->SetOrderAttack(pEnemy));
		// Dann bitte bewaffnen wenn nötig
		// Überhaupt Waffen da?
		if (!pTarget->IsFullyArmed() && FindObject2(Find_Func("IsFightWeapon"),Find_Distance(500,GetX(pTarget),GetY(pTarget)),Find_Not(Find_Container(pTarget)),Find_OCF(OCF_Available)) ) return(pTarget->SetOrderArm());
		// Überhaupt Feind da?
		if (pEnemy) return(pTarget->SetOrderAttack(pEnemy));
	}
	// Ansonsten nix ändern
	return();
  }
  
  // Andere Clonks greifen jetzt einfach an.
  else if(pEnemy) SetCommand(pTarget, "Attack", pEnemy);
