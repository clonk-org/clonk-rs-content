...
private func MagicMenu(object pByClonk) {
  ...
  CreateMenu(MCMS,pByClonk, 0, iExtra, "$MnuNoSpells$", GetMagicEnergy(pByClonk));
  var idMagic, i = 0;
  while (idMagic = GetPlrMagic(GetOwner(pByClonk), 0, i++)) 
    AddMenuItem("$MnuEnchant$: %s", "SetMagic", idMagic, pByClonk,0,pByClonk);
}
...
private func DoMagic(object pByClonk) {
  ...
  // Zauber
  if(CreateObject(idSpell,0,0,GetOwner(pByClonk))->~Activate(this(),pByClonk))
