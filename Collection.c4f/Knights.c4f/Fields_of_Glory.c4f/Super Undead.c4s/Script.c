...
public func GiveKillCount(int killer,int killed)
{
	...
	if (GetPlayerTeam(killer) != GetPlayerTeam(killed))
		...
		if (kill[killer] !=3 && kill[killer] !=6)
			{
			if (FindObjectOwner(SKLT,killer))
				{
				var nsklt = (FindObjectOwner(SKLT,killer));
				if(GetPlayerTeam(killer) == 1)
					{
					var jx=LandscapeWidth()/2 - 550 + RandomX(-20,20);
					var jy=LandscapeHeight()/2 + 124;
					}
				if(GetPlayerTeam(killer) == 2)
					{
					var jx=LandscapeWidth()/2 + 400 + RandomX(-20,20);
					var jy=LandscapeHeight()/2 + 164;
					}
				if (FindObjectOwner(_SSR,killer))
					{
				nsklt->Redefine(_SKN);
					}
				else
					{
				nsklt->Redefine(_SSR);
				nsklt->SetPhysical("Magic", 100000, 2);
					}
				}
			else
				{
				...
				var nsklt = CreateObject(SKLT,jx,jy,killer);
				MakeCrewMember(nsklt, killer);
				CreateContents(SRL1,nsklt);
				}
			}
		}
