...
// Aufgerufen von der KI, die den Schalten wieder zurücksetzen will
public func ControlReset(object pClonk)
{
  if(fStartDir == 1 && GetPhase()==1) return ControlLeft(pClonk);
  if(fStartDir == 2 && GetPhase()==0) return ControlRight(pClonk);
}
