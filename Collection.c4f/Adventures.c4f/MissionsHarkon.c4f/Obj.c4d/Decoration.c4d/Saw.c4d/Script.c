...
public func ProductionMenu(pWorker) {
  ...
  i = 0;
  while (def = GetPlrKnowledge(GetOwner(pWorker), 0, i++, C4D_Vehicle()))
    if (def->~IsSawProduct())
      AddMenuItem("$TxtProduceMenu$","ProductionOrder",def,pWorker,0,pWorker);
  // Fertig
  return(1);
...
private func Sawing() {
	if(GetID(FindCrewContents()) == _LBJ) { SetGraphics("Lumberjack"); FindCrewContents()->PutLogDown();}
	else SetGraphics();
  // Noch nicht fertig
...
private func ProductionCancel() {
  // Arbeit abbrechen
  if (GetAction() eq "Saw") SetAction("Idle");
  // Clonk(s) auswerfen
  var clnk;
