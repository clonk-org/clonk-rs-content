...
func Attach()
{
  ...
  if(pLayer) {
	...
	SetPosition(pObj->GetX()+iX, pObj->GetY()+iY);
	SetYDir(); SetXDir();
}
...
global func PosCam() // iX, iY oder pObj
{
  ...
  if(GetType(Par())==C4V_C4Object)
  {
    ...
    if(Par(2))
		{
			g_pCamera->SetAction("Attach", Par());
			g_pCamera->Attach();
			return 1;
		}
		AddEffect("IntCamFollow", g_pCamera, 1, 2, g_pCamera);
    SetCommand(g_pCamera, "Call", g_pCamera, Par(), 0, 0, "AttachMe");
    var pLayer = Par()->GetObjectLayer();
    var iX = 0, iY = 0;
    if(pLayer) {
      iX = pLayer->~GetLayerOffset();
      iY = pLayer->~GetLayerOffset(1);
    } 
    return AddCommand(g_pCamera, "MoveTo", 0, GetX(Par())+iX, GetY(Par())+iY);
  }
...
{
  var pLayer = pCamTarget->GetObjectLayer();
  var iX = pCamTarget->GetX(), iY = pCamTarget->GetY();
  if(pLayer) {
    iX += pLayer->~GetLayerOffset();
    iY += pLayer->~GetLayerOffset(1);
  } 
	if(GetAction() != "Attach" && Distance(GetX(), GetY(), iX, iY)>5)
	{
