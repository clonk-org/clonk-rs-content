...
local dlg_arAskedQuestions; // Schon gefragte Fragen
...
global func GetDialogControler(idID, szName)
{
	var pObj = FindCrewByName(idID, szName);
	if(!pObj) return 0;
	pObj = FindObject2(Find_ID(DG69), Find_ActionTarget(pObj));
	return pObj;
}

global func SetLocalNDialogs(szName, value)
{
	for(var pObj in FindObjects(Find_ID(DG69)))
		pObj->LocalN(szName) = value;
}
...
protected func ActivateEntrance(object speaker)
  ...
  // Ignorieren für Nicht-Dialoge
  if (GetAction() != "Attach") return;
  // KI? die dürfen nicht sprechen
  if(GetEffect("AI", speaker)) return;
  // Ziel verloren?
  if (!GetActionTarget()) { RemoveObject(); return; }
  ...
  // Haben wir unsern Dialogstring verloren? Einfach den Namen vom Clonk nehmen
  dlg_name = GetTargetName();
  // Kein Dialog?
...
private func GetTargetName()
{
  var szName = GetName(GetActionTarget());
  var c,i,p2="";
  while (c=GetChar(szName, i++))
  {
    if(c == GetChar("ö")) c = GetChar("o");
    if(c == GetChar("ä")) c = GetChar("a");
    if(c == GetChar("ü")) c = GetChar("u");
    if(c == GetChar("Ö")) c = GetChar("O");
    if(c == GetChar("Ä")) c = GetChar("A");
    if(c == GetChar("Ü")) c = GetChar("U");
    p2 = Format("%s%c", p2, c);
...
private func IsPlrInMenu()
  ...
  if (GetMenu(cursor) == WIPF) return true;
  // Ringmenü
...
global func Find_PlayerType(int plrtype) {
	var p = [C4FO_Or];
...
private func DlgMsg(string message, object speaker, string portrait, bool fPermanent)
  {
  // Portrait-Fallback
  ...
  return MsgBox(dlg_plr, message, speaker, portrait, 0, fPermanent);
  }

private func SetMsgPortrait(string new_portrait) { dlg_npc_portrait = new_portrait; }

private func IsQArrayAssigned() { if (dlg_arAskedQuestions) return true; }
  
protected func DlgQOption(string sCommand, int iQIdx)
  {
  // Callback des Menüs mit angegebenem iQIdx: Frage als gefragt markieren und entsprechenden Befehl aufrufen
  if (!IsQArrayAssigned()) dlg_arAskedQuestions = CreateArray(iQIdx+4);
  ++dlg_arAskedQuestions[iQIdx-1];
  return Call(sCommand, iQIdx);
  }
  
private func ResetAllQ(int iNum)
  {
  // Antwortarray zurücksetzen
  if (iNum>0)
    dlg_arAskedQuestions = CreateArray(iNum);
  else
    dlg_arAskedQuestions = 0;
  return true;
  }
  
private func IsQAsked(int idx)
  {
  if (IsQArrayAssigned() && idx>0) return (dlg_arAskedQuestions[idx-1]);
  }
  
private func & QAsked(int idx)
  {
  if (idx<=0) return;
  if (!IsQArrayAssigned()) dlg_arAskedQuestions = CreateArray(idx+4);
  return (dlg_arAskedQuestions[idx]);
  }

private func DlgAddOption(icon, string option, string command, param, int iQIdx, int extra, xPar)
  {
  if (iQIdx>0)
    {
    // Wurde schon gefragt?
    // vParam markiert hier, ob die Frage trotzdem mehrfach gefragt werden darf - dann wird nur gezählt
    if (IsQArrayAssigned() && !param) if (dlg_arAskedQuestions[iQIdx-1]) return;
    // Index aber wurde noch nicht gefragt: Erneut fragen - 8ung, hier gehen nur Befehlsnamen :(
    command = Format("DlgQOption(\"%s\", %d)", command, iQIdx);
    }
  // Warten
  dlg_waittime = DG69_Wait_Menu;
  // Icon auflösen
  if (GetType(icon) == C4V_C4Object)
    {
  ...
  if (!dlg_currmsgs || !GetLength(dlg_currmsgs))
    ...
    for(var part in dlg_currmsgs)
      ...
      if (i++)
        ...
        if (i == dlg_curroption) color = RGB(255,0,0);
...
global func MsgBox(object pPlayer, string sMessage, object pSpeaker, string szPortrait, bool fAsMessage, bool fPermanent)
  {
  // Defaults
  ...
  if (!szPortrait)
	{
		...
		{
			if(pSpeaker->GetPortrait(0))
			{
				szPortrait = pSpeaker->GetPortrait(0);
			}
			else
				szPortrait = "1";
		}
  ...
  if (fAsMessage)
    ...
    StopClonkEx(pPlayer);
    if (!CreateMenu(DG69, pMsgTarget, this, 0, "", 0, C4MN_Style_Dialog, fPermanent, DG69)) return false;
...
global func StopClonkEx(object target, bool keep_action)
  ...
  if (!keep_action)
    ...
    if (!WildcardMatch(GetAction(target), "Ride*"))
      {
      if (target) ObjectSetAction(target, "Walk"); else SetAction("Walk");
      }
    }
  StopClonk(target);
  CloseMenu(target);
