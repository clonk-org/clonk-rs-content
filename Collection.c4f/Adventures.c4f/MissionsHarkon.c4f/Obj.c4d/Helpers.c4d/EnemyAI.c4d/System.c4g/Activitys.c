...
func ExecMoveTo(iXOffset, iYOffset, iDir, pLayer)
{
	if(!GetCommand(this) && !GetMacroCommand(this) && Abs(GetX()-iXOffset) < 8 && Abs(GetY()-iYOffset) < 20)
	{
		if(iDir != -1)
		...
		SetMacroCommand("MoveTo", 0, iXOffset, iYOffset, pLayer);
	return 0;
}
...
func ExecGrab(pItem, iXOffset, iYOffset)
{
	...
	else if(!GetCommand(this) && !GetMacroCommand(this))
	{
		if(!iXOffset && !iYOffset)
			...
			SetMacroCommand("MoveTo", 0, pItem->GetX()+iXOffset, pItem->GetY()+iYOffset, pItem->GetObjectLayer());
	}
	return 0;
}
	...
	if(ExecGrab(pItem))
	{
		...
		if(ExtraGetValue(aExtra, 1))
		{
			...
			if(ExtraGetValue(aExtra, 2) > 0)
			{
				...
				{
					var pObj = CreateObject(ExtraGetValue(aExtra, 1));
					if(pItem->RejectCollect(pObj->GetID(), pObj))
					{
						pObj->RemoveObject();
						break;
					}
...
{//Log("%v StopActivityItem %v %v", this, pItem, aExtra);
	var idID = NONE;
	if(GetType(pItem) == C4V_C4Object) idID = GetID(pItem);
	if(GetType(pItem) == C4V_C4ID) idID = pItem;
...
func ExecActivityItem(pItem, aExtra) // 1 for keep 0 for finish
{
	...
	if(idID == BED1 || idID == BED2) // Bett
	{
		if(GetLength(LocalN("aZeiten")))
		{
			if(Contained() == pItem)
			{
				if(GetLifeState() != 2)
			...
			return Action_UseEnterItem(pItem, "Occupied", 0);
		}
		return Action_UseEnterItem(pItem, "Occupied", ExtraGetValue(aExtra, 1, 35*3));
	}
	
	if(idID == CHST || idID == WODC || idID == LORY || idID == CLD2 || idID == CLD1 || idID == SWRC) // Truhe
	...
	if(idID == PROC) // Process
  {
		var pos = ExtraGetValue(aExtra, 1, [GetX(), GetY(), GetDir(), GetObjectLayer()]);
		if(iActItemState == 0 && ExecMoveTo(pos[0], pos[1], pos[2], pos[3]))
		{
      ...
      AddEffect("IntCheckProcess", this, 1, 1, this);
			SetAction("Process");
			iActItemState = 1;
			return 0;
		}
  ...
  if(idID == STMN) // Strohmann
  {
    if(iActItemState == 0 && ExecMoveTo(pItem->GetX()-10+20*pItem->GetDir(), pItem->GetY(), 1-pItem->GetDir()))
    {
      AddEffect("IntWait", this, 1, 35*10+1);
      SetAction("Fight", pItem);
      pItem->SetAction("Fight", this);
      iActItemState = 1;
      return 0;
    }
    if(iActItemState == 1)
  ...
  if(idID == MTBL) // Mounted Bell
  {
    if(iActItemState == 0 && ExecMoveTo(pItem->GetX(), pItem->GetY(), 1-pItem->GetDir()))
    {
      AddEffect("IntWait", this, 1, 15+1);
      pItem->StartSwing();
      return 1;
    }
    return 0;
  ...
  if(idID == MOVE)
	{
		var pos = ExtraGetValue(aExtra, 1, [GetX(), GetY(), GetDir(), GetObjectLayer()]);
		return ExecMoveTo(pos[0], pos[1], pos[2], pos[3]);
	}
	if(idID == FOLW)
	{
	...
	if(idID == LOOK) // Umschauen
  {
	...
	if(idID == ALRM) // Renne zu einem Alarm hin
	{
		...
		if(!GetCommand() && !GetMacroCommand())
			SetMacroCommand("MoveTo", 0, aExtra[1], aExtra[2], aExtra[4]);
		return 0;
	}
	if(idID == _CIN) // Münze
	{
...
/*			SetComDir(COMD_Stop);
			...
			SetMacroCommand("MoveTo", 0, aExtra[1]->GetX()-10+10*aExtra[1]->GetDir(), aExtra[1]->GetY(), aExtra[1]->GetObjectLayer());
...
//				AddEffect("IntWait", this, 1, 35);
	...
	if(idID == MORN) // Betrauern
	{
		...
		if(iActItemState == 0 && ExecWalkTo(aExtra[1]))
		{
			...
			for(obj in FindObjects(Find_OCF(OCF_CrewMember), Find_Exclude(this), Find_InRect(-30,-20,60,40)))
			{
				...
				if(pAct && GetValueID(pAct) == MORN && LocalN("iActItemState", obj)>0)
					return 0;
			}
			SetMacroCommand("MoveTo", 0, aExtra[1]->GetX()-10+10*aExtra[1]->GetDir(), aExtra[1]->GetY(), aExtra[1]->GetObjectLayer());
			iActItemState = 1;
	...
	if(idID == DEAD)
	{
		...
		if(iActItemState == 1)
		{
			if(ExecMoveTo(GetX(pActTarget), GetY(pActTarget), GetDir(pActTarget), pActTarget->GetObjectLayer()))
			{
				iActItemState = 2;
	...
	if(idID == GRAV)
	{
      ...
      if(!pGrave) pGrave = FindObject2(Find_ID(1GRV), Find_Action("Occupied"), Sort_Random());
			if(!pGrave) { Log("ERROR: No grave found!"); return 1; }
			iActItemState = 1;
      ...
      if(!pDead) return 1;
			if(ExecMoveTo(GetX(pDead), GetY(pDead), GetDir(pDead), pDead->GetObjectLayer()) && GetProcedure() == "WALK")
			{
				iActItemState = 2;
      ...
      if(!pGrave) pGrave = FindObject2(Find_ID(1GRV), Find_Action("Free"), Sort_Random());
      if(!pGrave) pGrave = FindObject2(Find_ID(1GRV), Find_Action("Occupied"), Sort_Random());
      if(!pGrave) { Log("ERROR: No grave found!"); return 1; }
			if(ExecMoveTo(GetX(pGrave), GetY(pGrave), GetDir(pGrave), pGrave->GetObjectLayer()))
			{
				iActItemState = 3;
				ObjectCall(this, "PutDownCorps");
				AddEffect("IntWait", this, 1, 12*2);
	...
	if(idID == WAKE) // Schlafenden Kollegen aufwecken
	{
		...
		if(iActItemState == 0)
		{
			if(ExecMoveTo(GetX(pTarget), GetY(pTarget), GetDir(pTarget), pTarget->GetObjectLayer()))
			{
				iActItemState = 1;
	...
	if(idID == CORP) // Toten Kollegen entdecken
	{
		...
		if(iActItemState == 2)
		{
			...
			{
				SetActivity([NOTY, [GetX(), GetY(), GetDir(), GetObjectLayer()], pTarget]);
				SetActivity([LOOK, [GetX(), GetY(), GetDir(), GetObjectLayer()], pTarget]);
			}
      return 0;
		}
	}
...
func FxIntCheckProcessTimer(pTarget, iNumber)
{
  if(pTarget->GetAction() != "Process")
  {
    SetGraphics(0, this, 0, 23, 0);
    return -1;
  }
...
local fIsMourner;
public func IsMourner() { return fIsMourner; }
...
func FxIntDeadAttracktMournersTimer(pTarget, iNumber, iTime)
{
	...
	// Suche Trauergast
	for(var pObj in FindObjects(Find_OCF(OCF_CrewMember), Find_Owner(GetOwner(pTarget)), Find_Action("Walk"), Find_Func("IsMourner"), Sort_Random()))
	{
...
{
	if(GetObjectLayer() == obj->GetObjectLayer())
