...
local ai_iHomeX, ai_iHomeY, ai_iHomeDir, ai_pHome, ai_pHomeLayer; // Eingenommene Position, wenn kein Gegner gefunden - ai_iHomeX==0 für einfach stehen bleiben.
...
protected func AITimer(pNewTarget)
  {
...
//  var pNewTarget;
  ...
  if(ai_pTarget && (!GetAlive(ai_pTarget) || GetMacroPathLength(ai_pTarget->GetX(), ai_pTarget->GetY()) > ai_iRange)) ai_pTarget = 0;
...
//        CreateContents([XARP, FARP][Random(3)]);
  ...
  var pContainer;
  if ( (pContainer = Contained()) && !GetCommand() && !GetMacroCommand() && (!iCurrentActivity || ai_pTarget))
    {
    ...
    while (i--)
      ...
      if(GetMaterialVal("Extinguisher", "Material", GetMaterial(dx,dy)))
        if (dist = GetPathLength(GetX(), GetY(), GetX()+dx, GetY()+dy))
          if (dist < 800)
            return AI_SetCommand("MoveTo", 0, GetX()+dx, GetY()+dy, GetObjectLayer());
      }
...
//DebugLog(Format("%s #%d: AI_ExecMovement will follow", GetName(), ObjectNumber()), "AI_ExecMovement");
  ...
  if (!AI_IsRanged() || (ai_dwFlags & EA69_ForceMelee) || 1)
    ...
    if (GetProcedure() == "HANGLE")
      return AI_SetCommand("MoveTo", 0, GetX(), GetY()+20, GetObjectLayer());
...
protected func FxAIStop(object pTarget, int iEffectNumber, int iReason, bool fTmp)
  ...
  if(ai_DialogSound) ai_DialogSound->FinishSound();
  ...
  {
		if(aStealItems && GetIndexOf(obj, aStealItems)!=-1)
		{
			i++;
			continue;
		}
    if(GetEffect("IntDeathRemoveMarker", obj) || obj->~IsArrowPack() || obj->~IsArrow() || GetID(obj)==BOW1 || GetID(obj)==AXE1 || GetID(obj) == SWOR || GetID(obj) == SPER) RemoveObject(obj);
    else i++;
  }
...
//  return;
  ...
  if(szString)
  {
    ai_DialogSound = CreateObject(_CAM);
    ai_DialogSound->SetSound(szString, this);
  }
}
...
private func AI_Home()
  {
	if(!ai_iRange) return;
  if( aListActivitys || GetLifeState() != 0 || GetLength(iCurrentActivity))
  ...
  if (Abs(GetX() - ai_iHomeX) > 10 || Abs(GetY() - ai_iHomeY) > 30) if (!ai_pHome && ai_iHomeX)
    ...
    AI_SetCommand("MoveTo", 0, ai_iHomeX, ai_iHomeY, ai_pHomeLayer);
  ...
  if( ai_aPatrolPoints && (!Random(20+40*!!(Contained())+40*!!aStealItems ) || ai_fRunner) )
  {
    ...
    if(GetType(ai_aPatrolPoints[ai_iPatrolIndex])==C4V_C4Object)
		{
			...
			ai_pHomeLayer = GetObjectLayer(ai_aPatrolPoints[ai_iPatrolIndex]);
		}
		else
		{
			...
			ai_iHomeDir = 0;//ai_aPatrolPoints[ai_iPatrolIndex][2];
			if(ai_iHomeX > GetX()) ai_iHomeDir = 1;
			ai_pHomeLayer = ai_aPatrolPoints[ai_iPatrolIndex][3];
		}
...
private func AI_ExecThrow()
  ...
  if (pProjectile = FindContents(SPER))
    ...
    if (!(pProjectile = FindObject2(Find_Container(this), Find_OCF(OCF_OnFire))))
      ...
      for(;;)
        ...
        if(aStealItems && GetIndexOf(pProjectile, aStealItems) < 0) break;
  ...
  if (!Random(5)) return AI_SetCommand("Throw", pProjectile, GetX(pEnemy), GetY(pEnemy), GetObjectLayer(pEnemy));
...
private func AI_ExecFight()
  ...
  if (!Random(15))
    ...
    AI_SetCommand("MoveTo", 0, GetX()+(GetX() >= GetX(pOpp))*50-25, GetY(), GetObjectLayer());
    return AI_ExecMovement();
    }
  return true;
  }
...
private func ExecuteAttackBowX(pTarget)
  ...
  var iTx = GetX(pTarget), iTy = GetY(pTarget);
	if(!FireBowAtX(iTx, iTy, true))
		return 0;
  // Ziel in Reichweite?
  if (Abs(iTx-GetX())>300 || Abs(iTy-GetY())>300 || GetObjectLayer() != pTarget->GetObjectLayer())
	{
		if(!GetMacroCommand())// || !Random(5))
    // Nicht? Mögliche Abschußposition einnehmen
    return( SetMacroCommand("MoveTo",0, BoundBy(GetX(),iTx-290,iTx+290),GetY(), pTarget->GetObjectLayer()) );
		return 1;
	}
  // Bogen auswählen
...
private func FireBowAtX(int iX, int iY, bool fTest, idProjectile)
{
	var dx = iX-GetX();
	var dy = iY-GetY();
  ...
  AimBowToAngle(angle);
  // Feuern
  return(Contents()->~FireSpeed(vx, vy, angle, idProjectile));
} 
...
private func GetWarning(idType) {
	return FindObject2(Find_ID(idType), Find_ActionTarget(this));
}
...
private func SetWarning(idType) {
  var pWarn=GetWarning(idType);
  if(!pWarn) return ObjectSetAction(CreateObject(idType),"WarnAction",this,this);
}

private func RemoveWarnings() {
  RemWarning(WNWT); RemWarning(WNST); RemWarning(WNTK);
}

private func AI_FindNearestEnemy(fLookAround)
  {
  // Sucher
  if(fLookAround || ai_fFindAssassins || ai_iAlarmedCounter) SetWarning(WNWT);// Message("{{_EYE}}", this);
  else RemWarning(WNWT);
//  else Message("", this);
...
/*  AI_SetCommand("MoveTo", 0, GetX(pSource), GetY(pSource));
  ...
  ai_iActionType = 2;*/
	...
	SetActivity([ALRM, pSource->GetX(), pSource->GetY(), -1, pSource->GetObjectLayer()]);
  return 1;
}
...
public func GetLifeState() { return _inherited(...); }
// Kompatibilität
