...
protected func FxAITimer()
  ...
  var aProjectiles = FindObjects(Find_InRect(-50,-50,100,80), Find_Or(Find_Category(C4D_Object), Find_Func("IsDangerous4AI"), Find_And(Find_Func("IsArrow"), Find_Hostile(GetOwner()))), Find_OCF(OCF_HitSpeed2), Find_NoContainer());
  ...
  // Projektilen ausweichen
    ...
    if (dx*vx+dy*vy < 70*Sqrt(dx*dx+dy*dy)*Sqrt(vx*vx+vy*vy)/-100)
      ...
      if (pContainer = FindObject2(Find_AtPoint(), Find_OCF(OCF_Entrance), Find_Not(Find_Func("IsHorse")), Find_Not(Find_ID(CAVE))))
  ...
  if (!ai_pTarget && !ai_pActionTarget) return AI_Home();

  // Aktion
  if(ai_pActionTarget) return AI_ExecAction();
...
private func AI_ExecAction()
{
  if(ObjectDistance(ai_pActionTarget)<10)
  {
    if(ai_iActionType == 0 && !GetEffect("SleepNSpell", ai_pActionTarget))
    {
      if(ai_iActionIndex < 20)
      {
        if(ai_iActionIndex == 0)
          MessageEx(GetStringHurt(), this, GetName(ai_pActionTarget));
        ai_iActionIndex++;
      }
      else
      {
        if(ai_iActionIndex == 20)
          MessageEx(GetStringDead(), this, GetName(ai_pActionTarget));
        ai_iActionIndex++;
        if(ai_iActionIndex < 40)
          ai_iActionIndex++;
        else
        {
					ai_iAlarmedCounter = 10;
					var pNewTarget = AI_FindNearestEnemy(1);
					if (ai_idFirstEncounterCB) AI_DoEncounter(ai_pActionTarget, 1);
					// Nur wenn ein neuer Gegner da ist!
					if(pNewTarget)
					{
						MessageEx(GetStringSeenMurder(), this, GetName(ai_pActionTarget));
						ai_iAlarmedCounter = 0;
						pNewTarget->~DoShow(250);
						ai_pTarget = pNewTarget;
					}
					...
					{
						SetActivity([NOTY, [GetX(), GetY(), GetDir()], ai_pActionTarget]);
						SetActivity([LOOK, [GetX(), GetY(), GetDir()], ai_pActionTarget]);
					}
          ai_iActionIndex = 0;
          ai_pActionTarget = 0;
        }
    ...
    if(ai_iActionType == 0 && GetEffect("SleepNSpell", ai_pActionTarget))
    {
      RemoveEffect("SleepNSpell", ai_pActionTarget);
			MessageEx(GetStringWakeUp(), this, GetName(ai_pActionTarget));
      ai_pActionTarget = 0;
    }
    if(ai_iActionType == 1)
    {
      if(ai_iActionIndex == 0)
      {
        if(GetID(ai_pActionTarget) == _SWT && ai_pActionTarget->~ControlReset())
          MessageEx(GetStringSwitch(), this);
				if(GetID(ai_pActionTarget) == _CIN)
  ...
  else if(!GetCommand() && !GetMacroCommand())   AI_SetCommand("MoveTo", 0, GetX(ai_pActionTarget), GetY(ai_pActionTarget));//ai_pActionTarget = 0;
}
...
//  else Message("", this);
  ...
  for (var obj in FindObjects(Find_Distance(ai_iRange), cont, Find_OCF(OCF_Alive), Sort_Distance(), Find_Hostile(GetOwner()), Find_Layer(GetObjectLayer())))
	...
	if(GetLength(iCurrentActivity) == 0)	
	// Münzen suchen
...
public func AI_LookForFriend(pFriend)
{
  ...
  AI_SetCommand("MoveTo", 0, GetX(pFriend), GetY(pFriend));
  ai_pActionTarget = pFriend;
  ai_iActionIndex = 0;
  ai_iActionType = 0;
...
global func AI_AlarmCurious(pSource, iRange)
{
  ...
  for(var obj in FindObjects(Find_Distance(iRange, GetX(pSource)-GetX(), GetY(pSource)-GetY()), Find_OCF(OCF_Living), Find_OCF(OCF_Alive), Sort_Distance()))
    if (obj->~IsAI())
