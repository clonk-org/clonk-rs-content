...
static const Path_Scale = 7;
static const Path_Ladder = 8;
...
global func WaypointsVisible(bool visu) {
  for(var obj in FindObjects(Find_ID(WAYP)))
    obj->SetVisible(visu);
	for(var obj in FindObjects(Find_ID(_WPS)))
		obj->SetVisible(visu);
...
{
	var i = GetPathToTarget(pEnd);
	if(i != -1) return GetPathTarget(i);
...
public func SetPathExtra(int pId, pExtra) {
  if(GetPathCount() <= pId) return ErrorLog(WAYP_PathErr, pId);

  EffectVar(4,this(),aPath[pId]) = pExtra;
}
...
public func GetPathExtra(int pId) {
  if(GetPathCount() <= pId) return ErrorLog(WAYP_PathErr, pId);
	if(!EffectVar(4,this(),aPath[pId]))
		EffectVar(4,this(),aPath[pId]) = FindExtra(GetPathFlag(pId), GetPathTarget(pId));
  return EffectVar(4,this(),aPath[pId]);
}

public func SetPathExtra(int pId, pObj) { EffectVar(4,this(),aPath[pId]) = pObj; }

func FindExtra(iType, pNextWp)
{
	if(iType == Path_Stairs)
	{
		var direction;
		var wait = 0;
		if(GetY(pNextWp) < GetY()) direction = "ContainedUp";
		else direction = "ContainedDown";
		var stairs = FindObject2(Find_Func("IsStaircase"), Find_AtRect(-25, -25, 50, 50));
		if(!stairs) { direction = 0; stairs = FindObject2(Find_ID(CSTE),Find_AtRect(-25, -25, 50, 50)); }
		if(!stairs) { wait = 35; direction = 0; stairs = FindObject2(Find_ID(WGTW),Find_AtRect(-25, -25, 50, 50)); }
		if(!stairs) { stairs = FindObject2(Find_Func("IsDoor"), Find_AtRect(-25, -25, 50, 50)); wait = stairs->~GetWait(); direction = stairs->~GetDirection(); }
		if(stairs)
			return [stairs, direction, wait];
	}
	if(iType == Path_Lift)
	{
	...
	if(iType == Path_Door)
	{
		var x1 = GetX();
		var x2 = pNextWp->GetX();
		if(x1 > x2) { x1 = x2; x2 = GetX(); }
		var door = FindObject2(Find_Func("IsDoor"), Find_AtRect(x1-GetX(), -25, x2-x1, 50));
    if(!door) door = FindObject2(Find_ID(CPSG), Find_AtRect(x1-GetX(), -25, x2-x1, 50));
		if(door) return [door, 0];
		// Spezialbehandlung für Lagertürme
		door = FindObject2(Find_ID(WGTW), Find_AtRect(-5, -50, 10, 100));
		if(door && GetY()>GetY(door) && Abs(GetX()-GetX(door)) < 5)
		{
			if(GetX() < GetX(pNextWp))
				return [door, COMD_Right];
			return [door, COMD_Left];
		}
		door = FindObject2(Find_ID(WGTW), pNextWp->Find_AtRect(-5, -50, 10, 100));
		if(door && GetY(pNextWp)>GetY(door) && Abs(GetX(pNextWp)-GetX(door)) < 5)
		{
			if(GetX() > GetX(pNextWp))
				return [door, COMD_Right];
			return [door, COMD_Left];
	...
	if(iType == Path_Scale)
	{
		var helper = FindObject2(Find_ID(_WPS), Sort_Distance(), Find_Distance(300));
	}
}
