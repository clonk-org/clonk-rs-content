...
func Initialize()
{
	...
	if(!FindObject(GetID()))
	{
		...
		SetPosition(16,16);
		aTools = [0,0,0,0,0,0];
		for(var i = 0; i < GetToolCount()+2; i++)
		{
...
func UpdateTool()
{
	iMode = [-1, Path_MoveTo, Path_Jump, Path_Scale, Path_Lift, Path_Door, Path_Stairs, Path_Fly, Path_Remove][iTool];
	for(var i = 0; i < 8; i++)
		aTools[i]->SetDir(0);
...
func ControlThrow()
{
	...
	aTools[GetToolCount()+1]->SetDir(iReconnect);
	if(lastPos) SetPlrView(GetOwner(), lastPos);
}
