...
public func ActivateAngle(object pCaller, int iAimAngle)
{
  ...
  SetAction("Travel");
  SetPosition(GetX(pCaller)+Sin(iAimAngle, 20),GetY(pCaller)-Cos(iAimAngle, 20));
  SetXDir(Sin(iAimAngle, 50));
  SetYDir(-Cos(iAimAngle, 50));
  SetCategory(4);
  iOldX = GetX(); iOldY = GetY();
  return 1;
}
...
//  CastPXS("Snow",RandomX(8,12),RandomX(20,30));
  CastParticles ("MagicFire", RandomX(1,3), RandomX(20,30), 0, 0, 50, 250, RGB(255,255,255), RGB(255,255,255));
...
func Hit() {
  CastParticles ("MagicFire", RandomX(8,12), RandomX(20,30), 0, 0, 10, 50, RGB(255,255,255), RGB(255,255,255));
