...
{
	if(idObj == BOW1 || idObj == SWOR || idObj == SPER || idObj == AXE1 || idObj == ARWP || idObj == ARRW || idObj == FARP || idObj == FARW || idObj == XARP || idObj == XARW) return 1;
...
public func CheckSaneFight()
{
  if(GetActionTarget() == this) { return SetAction("Walk"); }
  return _inherited(...);
}
/*
local iChopTimer;
...
private func Chopping()
{
  if (!GetActTime())
  {
    if(!GetEffect("IntNoChopDmg", GetActionTarget())) AddEffect("IntNoChopDmg", GetActionTarget(), 1, 0, this);
    return iChopTimer = 0;
  }
  iChopTimer++;
  if(iChopTimer >= 4)
  {
    CreateObject(WOOD);
    iChopTimer = 0;
    GetActionTarget()->DoCon(-30);
  }
  return _inherited(...);
}

func FxIntNoChopDmgDamage(pTarget,iEffectNumber,iDmg,iCause)
{
  if(iCause == FX_Call_DmgChop) return 0;
  return iDmg;
}*/
...
{
  // nur im Stehen
  if(GetAction() != "Walk") return _inherited();
...
//  if(FindObject(CNSW)) // Nur mit Regel
  while( pTree = FindObject(0,0,1,0,0,OCF_Grab,0,0,NoContainer(),pTree) )
    if(pTree->~IsTree())
      return SetAction("Chop2",pTree) || 1;
  return _inherited();
}
...
public func ControlDown()
{
  ...
  if (GetAction() == "Chop2") return StopAction();
...
public func GetMaxSpecialCount(iIndex, fAmount)
{
  ...
  if(iIndex == 2) { if(fAmount) return  0; return "IsArrow"; }
  if(iIndex == 0) { if(fAmount) return 30; return "IsBolt"; }
  if(iIndex == 1) { if(fAmount) return  5; return "IsKey"; }
}
...
func FxHideStart(pTarget, iNumber, fTmp, fInstantHide)
{
  ...
  SetPhysical("Walk",GetPhysical("Walk", 0,pTarget)-15000,PHYS_StackTemporary,pTarget);
  SetPhysical("Scale",GetPhysical("Scale", 0,pTarget)-10000,PHYS_StackTemporary,pTarget);
  SetPhysical("Hangle",GetPhysical("Hangle", 0,pTarget)-10000,PHYS_StackTemporary,pTarget);
...
func FxHideTimer(pTarget, iNumber)
{
  ...
  if(FindContents(RPND)) EffectVar(3, pTarget, iNumber) = 50; // Sichtbarkeit wenn Seilende in der Hand
...
public func IsNotHidden() { if(!IsDisguised()) return !fHide; }
...
public func Assassasinated()
{
  ...
  {
		if(Contained() != pTarget->Contained())
			if(!pTarget->Contained() || !(pTarget->Contained())->~IsAssassinable())
				return 0;
    if(pTarget->~HasArmor())
    {
      Sound("ClonkHit*");
      DoEnergy(-30, pTarget);
      pTarget->~AITimer(this);
    }
    else
      Kill(pTarget);
		if(pTarget->Contained())
			(pTarget->Contained())->OnAssassination();
  }
}
...
{
  var pTarget = GetActionTarget();
  if(pTarget && ObjectDistance(pTarget)<22 && Contained() == pTarget->Contained())
  {
    if(pTarget->~HasArmor())
    {
      Sound("ClonkHit*");
...
func HasMoney() { return GetWealth(GetOwner()); }
...
func GetCorps(fRemove)
{
	...
	var obj = EffectVar(3, this, iEff);
	if(fRemove)
		RemoveEffect(0, this, iEff);
	return obj;
}

local szGraphicsName;
...
func FxCarryControlCorpsStart(pTarget, iNumber, fTmp, pCarry, pCarryHelper)
{
  ...
  SetPhysical("CanHangle", 0, PHYS_StackTemporary);
	EffectVar(0, pTarget, iNumber) = ObjectNumber(pCarry);
	EffectVar(1, pTarget, iNumber) = ObjectNumber(pCarryHelper);
        //damit GetCorps funktioniert
        EffectVar(3, pTarget, iNumber) = pCarry;
	if(!pCarryHelper)
	{
		pCarryHelper = pCarry->CreateObject(_CYH, 0, 10, -1);
		EffectVar(1, pTarget, iNumber) = ObjectNumber(pCarryHelper);
		pCarry->Enter(pCarryHelper);

		if(ObjectSetAction(pCarry, "BeingTakenUp", 0, 0, 1))
		{
			EffectVar(2, pTarget, iNumber) = ObjectNumber(pCarry);
		}
		else
		{
			EffectVar(2, pTarget, iNumber) = EffectVar(1, pTarget, iNumber);
			
			pCarryHelper->SetAction("BeingTakenUp", this);
			pCarryHelper->SetColorDw(pCarry->GetColorDw());
			if(!pCarryHelper->SetGraphics(pCarry->GetName()))
				pCarryHelper->SetGraphics(Format("%i", pCarry->GetID()));
		}
		pCarryHelper->SetVisibility(VIS_OverlayOnly);
		SetAction("CarryTakeUp");
	}
	SetGraphics(0, this, 0, 11, GFXOV_MODE_Object, 0, 0, Object(EffectVar(2, pTarget, iNumber)));
	SetGraphics(szGraphicsName, this,GetID(), 20, GFXOV_MODE_ExtraGraphics);
}
...
func FxCarryControlCorpsTimer(pTarget, iNumber)
{
	...
	var pCarry = Object(EffectVar(0, pTarget, iNumber));
	var pHelper1 = Object(EffectVar(1, pTarget, iNumber));
	var pHelper2 = Object(EffectVar(2, pTarget, iNumber));
	if(!pCarry || !pHelper1 || GetID(pHelper1) != _CYH)
	{
		SetAction("Walk");
		return -1;
	}
	...
	{
		pHelper2->SetAction("GettingCarried", this);
		pHelper2->SetPhase(GetPhase());
	}
	...
	if(GetObjectLayer() != GetObjectLayer(pHelper2))
	{
		if(GetObjectLayer(pHelper2))
			GetObjectLayer(pHelper2)->RemObject(pHelper2);
		if(GetObjectLayer())
			GetObjectLayer()->AddObject(pHelper2);
	}
	if(GetObjectLayer() != GetObjectLayer(pHelper1))
	{
		if(GetObjectLayer(pHelper1))
			GetObjectLayer(pHelper1)->RemObject(pHelper1);
		if(GetObjectLayer())
			GetObjectLayer()->AddObject(pHelper1);
	}
	pHelper2->SetDir(GetDir());
	pHelper1->SetPosition(GetX(), GetY());
  // Erlaubte Aktionen
...
func FxCarryControlCorpsStop(pTarget, iNumber, fTmp)
{
  ...
  ResetPhysical(pTarget, "CanScale");
	...
	var pHelper1 = Object(EffectVar(1, pTarget, iNumber));
	var pHelper2 = Object(EffectVar(2, pTarget, iNumber));
	if(pCarry) pCarry->SetDir(GetDir());
  if(pHelper2) {
    pHelper2->SetAction("Dead");
    pHelper2->SetPhase(5);
  }
	if(pCarry && Contained(pCarry) == pHelper1)
	{
		if(Contained()) pCarry->Enter(Contained());
		else Exit(pCarry, 0, 10);
	}
	if(pHelper1) pHelper1->RemoveObject();
...
func ControlThrow()
{
	...
	if(fDisguised && !GetPlrDownDouble(GetOwner())) return 1;
	return _inherited(...);
}
...
{
  [$CtxHideDesc$|Image=CMFG|Condition=CanHide]
  // Tarnen
  Hide();
  return 1;
}
...
public func IsDisguised() { return fDisguised; }

public func SetDisguised(iIndex)
{
	fDisguised = 1;
	if(iIndex == 1) SetGraphics("Hangman");
	UnHide();
	SetHostility(GetOwner(), 10, 0, 1, 1);
	SetHostility(10, GetOwner(), 0, 1, 1);
}

public func SetNotDisguised()
{
	fDisguised = 0;
	SetHostility(GetOwner(), 10, 1, 1, 1);
	SetHostility(10, GetOwner(), 1, 1, 1);
}

func FindHangman() { if(!IsDisguised()) return FindObject2(Find_ID(_GLM), Find_NoContainer(), Find_Not(Find_OCF(OCF_Alive)), Find_AtPoint()); }
...
func FindSteal() { if(GetAction() != "Walk") return 0;
	var aRect = Find_InRect(-40,-20,40,40); if(GetDir()) aRect = Find_InRect(0,-20,40,40);
  var pEnemy = FindObject2(Find_OCF(OCF_CrewMember), aRect, Find_Hostile(GetOwner()), Find_OCF(OCF_Alive), Find_Container(Contained()), Find_Func("CanBeStolen"));	
	if(!pEnemy) return 0;
  if(pEnemy->GetAction() != "Dead")
    if(GetDir(pEnemy)!=GetDir()) return 0;
	return pEnemy;
}
...
local aQuestItems;
local pQuestStorage;
public func GetQuestStorage() { return pQuestStorage; }
...
func AddQuestItem(pObj)
{
	if(!pQuestStorage)
	{
		pQuestStorage = CreateObject(BGST, 0, 0, GetOwner());
		pQuestStorage->SetBagID(pObj, this);
	}
	pObj->Enter(pQuestStorage);
  pQuestStorage->Highlight(pObj);
	pQuestStorage->UpdateQuestDisplay();
}
