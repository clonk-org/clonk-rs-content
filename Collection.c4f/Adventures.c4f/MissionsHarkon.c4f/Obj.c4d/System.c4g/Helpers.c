...
global func PoffClonk(pTarget)
{
  ...
  CastParticles("PSpark", 10, 8, GetX(pTarget)-GetX(), GetY(pTarget)-GetY(), 20, 180, GetColorDw(pTarget), GetColorDw(pTarget));
...
{
	if(FindObject2(Find_Distance(150), Find_PlayerType(C4PT_User), Find_OCF(OCF_CrewMember))
		|| FindObject2(Find_Distance(150), Find_ID(_SUL), Find_Action("")))
	{
...
{
	var obj = 0;
	if(obj = FindObject2(Find_Distance(EffectVar(2, pTarget, iNumber)), Find_PlayerType(C4PT_User), Find_OCF(OCF_CrewMember)))
	{
...
global func FxIntSetDirToTimer(pTarget, iEff) { SetDirTo(EffectVar(0, pTarget, iEff), pTarget); }
