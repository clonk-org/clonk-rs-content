...
// Ausweichen: CMFG, MCIB (Clonk in Flasche), MFFW, ABLA, GVTY (Schweben), MDFL (Zurückwerfen), MSSH (Steinschild), (MBRG)
// Werfen: FGOB (Schleudern), MINC (Anzünden)
// Löschen: EXTG
// Panic: CMFG
// Heilung: MGFO (Futter), ELX1, ELX2, MGHL
// Angriff: MLGT, ETFL, FRFS, ABLA, MBOT, MFRB, MDBT (Drachenatem), MFBL, MFWL, GZ9Z, LGCN, RUND
//          MTSK (Teraschock), AFST (Luftklaue), PRFT (Druckfaust), AUEQ (Erdbebenaura), MARK (Steinschlag), SHFT (SChattenfaust), AUHE (Hitzeaura)
//          MFCS (Feuercluster), FLFT (Flammenfaust), MFWV (Frostwelle), ICFT (Eisfaust), MGSW (Schneeball)
// Schwimmend: MCLD (Flüssigkeiten erstarren)
// Endboss: MGRP
// Ally-Hilfe: ELX2, ELX1, MCIB (Clonk in Flasche), MHGA (M&M-Gruppenheilung), GGHG
// Ally-Nahkampfunterstützung: GZ9Z, MDFL (Zurückwerfen), MSSH (Steinschild), MHTB (Heißes Blut)
//
...
static g_EA69_aNoThrowMage; // für Magier
static g_EA69_aNoThrowPaladin; // für Paladin
static g_EA69_aNoThrowPirate; // für Pirat
static g_EA69_aNoThrowKnight; // für Ritter
static g_EA69_aNoThrowSkeleton; // für Skelett

static const EA69_Diff_Easy   = 0,
              EA69_Diff_Medium = 1,
              EA69_Diff_Hard   = 2;

static g_iAIScriptPlayer;

local ai_iRange;                         // Suchweite für Gegner
local ai_iHomeX, ai_iHomeY, ai_iHomeDir, ai_pHome; // Eingenommene Position, wenn kein Gegner gefunden
...
local ai_iChurch;			 // Gewählte Kirche für den Paladin
...
local ai_fDoMelee;                       // Gesetzt für Fernkämpfer, die auch in den Nahkampf gehen
local ai_iDisableTime;                   // Verhindert KI-Aufrufe für die vorgegebene Zeit
local ai_pWeapon;			 // Fahrzeugwaffe, die die KI einsetzen soll (Kataplut, Haubitze)
local ai_fNoEvade;			 // Cheatige KI, die in Schaukämpfen zum Einsatz kommt
local idStaff;				 // Stab für Magier
local ai_aPatrolPoints;			 // Punkte zum Patrollieren
local ai_iPatrolIndex;			 // Index für das Patrollieren

/* Scriptspieler */
public func InitializeScriptPlayer(player)
{
  var iPlr;
  ...
  for(var i = 0; i < GetPlayerCount(); i++)
  {
      ...
      if(iPlr == player) continue;
      // Die Ki mag niemanden!!
      SetHostility(iPlr,player,1,1);
      SetHostility(player,iPlr,1,1);
  }
}
...
global func AI_InitSystem()
  ...
  g_EA69_aNoThrowMage = [PMAN, SCKZ, SMAR, SMER, SMFR, SMWT]; // für Magier
  g_EA69_aNoThrowPaladin = [TSWD, SBLP, PBLP, MUSK, BLTP]; // für Paladin
  g_EA69_aNoThrowPirate = [SABR, AXE1, DNGR, KNFE, SBLP, PBLP, MUSK, BLTP]; // für Pirat
  g_EA69_aNoThrowKnight = [SWOR, AXE1, BOW1, ARWP, FARP, XARP]; // für Ritter
  g_EA69_aNoThrowSkeleton = [SWOR, AXE1]; // für Skelett
  return true;
  }
...
protected func Initialize(a,b,c,d,e,f,g,h,i,j)
  ...
  // Impliziert auch magische Kopien, belebte Tote, etc.
  if (ObjectCount(EA69_RuleID) && GetOwner()<0 && !ai_iRange) ScheduleCall(this, "SetAI", 50, 1);
...
/*  if (!Random(10)) DoMagicEnergy(ai_iMagicRegen * (1+g_EA69_iDifficulty));
  ...
  if(GetOwner()==-1)
  {
    if(GetPlayerType(g_iAIScriptPlayer)==C4PT_Script)
    {
      var iColor = GetColorDw();
      SetOwner(g_iAIScriptPlayer);
      SetColorDw(iColor);
    }
    else
    {
      for(var i=0; i<GetPlayerCount(); i++)
      {
        ...
        if(GetPlayerName(iPlr)=="AI") {
          g_iAIScriptPlayer = iPlr;
	  var iColor = GetColorDw();
	  SetOwner(g_iAIScriptPlayer);
	  SetColorDw(iColor);
          break;
      ...
      if(GetOwner()==-1 && g_iAIScriptPlayer!=-1)
      {
        CreateScriptPlayer("AI", 0xff0000, 0, true);
	g_iAIScriptPlayer = -1;
      }
    }
  }
  ...
  var aProjectiles = [];
  if(!ai_fNoEvade) aProjectiles = FindObjects(Find_OCF(OCF_HitSpeed2), Find_NoContainer(), Find_InRect(-50,-50,100,80), Find_Or(Find_Category(C4D_Object), Find_Func("IsDangerous4AI")));
  ...
  // Projektilen ausweichen
    ...
    if (dx*vx+dy*vy < 70*Sqrt(dx*dx+dy*dy)*Sqrt(vx*vx+vy*vy)/-100)
      ...
      if (pContainer = FindObject2(Find_AtPoint(), Find_OCF(OCF_Entrance), Find_Not(Find_Func("IsHorse"), Find_ID(DG69))))
  ...
  if (OnFire() && !GetEffect("HotBloodPSpell", this))
    {
  ...
  // Magier: Zaubern
  if (AI_CanCast()) if (GetMagicEnergy() >= 15) if (AI_ExecMagician()) return true;

  // Werfen wenn nötig
  if(!IsMystic()) if (Contents()) if (AI_ExecThrow()) return true;
...
protected func FxAIStop(object pTarget, int iEffectNumber, int iReason, bool fTmp)
  ...
  var obj, i;
  if(obj = FindObject(SHIA, 0, 0, 0, 0, 0, 0, this)) RemoveObject(obj);
  while(obj = Contents(i))
  {
    if (GetOwner(obj)<0 && GetIndexOf(GetID(obj), [SPER, ROCK, FLNT, SFLN, EFLN, STFN, FBMP, BOW1, ARWP, ARRW, FARP, FARW, XARP, XARW, AXE1, SWOR, MUSK, TSWD, BLTP, PBLP, SBLP, DNGR, KNFE, SABR, CRBW]) >= 0) RemoveObject(obj);
    else i++;
  }
  if(IsKnight()) { LocalN("fArmored") = 0; LocalN("idArmorType") = 0; }
...
public func AddPatrolPoint()
{
  if(!ai_aPatrolPoints) ai_aPatrolPoints = [];
  ai_aPatrolPoints[GetLength(ai_aPatrolPoints)] = [GetX(), GetY()];
}
...
private func AI_Evade(int iDir, object pProjectile) // iDir in {-1,+1}
  ...
  if (pProjectile) if (GetMagicEnergy()>=9) if (AI_CanCast())
    ...
    if (!Random(5)) if (pProjectile->~IsArrow())
      ...
      var pShooter = GetCursor(GetController(pProjectile));
      if (pShooter) if (pShooter->~IsAiming())
        {
        var iYDiff = GetY(pShooter) - GetY();
        if (iYDiff > 20) if (iYDiff > Abs(GetX() - GetX(pShooter)))
          if (AI_DoMagic(MBRG, METL, 50)) return true;
        }
    ...
    // M&M: Schweben
    if (!Random(3)) if (pProjectile->~IsArrow())
  ...
  { AddCommand(0, "Jump", 0, iDir) && SetComDir(5-2*iDir); return AI_ExecMovement(); }
  // Weg frei?
  if (PathFree(GetX(), GetY(), GetX()+30*iDir, GetY()))
  { AddCommand(0, "MoveTo", 0, GetX()+30*iDir,GetY()); return AI_ExecMovement(); }
  // Nix frei :(
  }
...
private func AI_ExecThrow()
  ...
  if (pProjectile = FindContents(SPER))
	...
	++i;
        ...
        if (IsPaladin())	if (GetIndexOf(GetID(pProjectile), g_EA69_aNoThrowPaladin) >= 0) continue;
        if (IsPirate())		if (GetIndexOf(GetID(pProjectile), g_EA69_aNoThrowPirate) >= 0) continue;
        if (IsMagiclonk())	if (GetIndexOf(GetID(pProjectile), g_EA69_aNoThrowMage) >= 0) continue;
        if (GetID()==SKLT)	if (GetIndexOf(GetID(pProjectile), g_EA69_aNoThrowSkeleton) >= 0) continue;
        break;
        }
      }
    }
  ...
  if ((GetID(pProjectile) == TSWD) && IsPaladin())
    ...
    if (Inside(dy, -30, -5))
      {
      if (!(fThrow=Inside(Angle(0,0,dx,dy), 20, 60)))
        {
        if (!Random(2))
...
private func AI_ExecPaladin()
  ...
  if (AI_CanConjure()) if (!Random(3))
    ...
    if (!Random(6)) if (PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
      {
      // EnergyAttack
      if (ai_iChurch!=scBloodchurch) if (AI_DoConjure(MGPH)) return true;
      // Horizontaler Angriff: LifeDepletion
      if (ai_iChurch==scBloodchurch) if (Abs(GetY() - GetY(ai_pTarget)) < 30) if (AI_DoConjure(MLDL)) return true;
      }
    ...
    if (!Random(10))
      ...
      if (clnk)
        ...
        if (ai_iChurch!=scBloodchurch) if (AI_DoConjure(MRVL)) return true;
        // Wiederbelebung als Skelett
        if (ai_iChurch==scBloodchurch) if (AI_DoConjure(MSMN)) return true;
        }
      }
    // Schneller laufen
    if (GetComDir()) if (ai_iChurch==scBloodchurch) if (!Random(5)) if (!GetEffect("SpeedPSpell", this))if (AI_DoConjure(MSPD)) return true;
    }
  }
...
private func AI_ExecMagician()
  ...
  // Stabzauber verwenden
  if(idStaff)
  {
    if(idStaff==SMAR) // Luftzauber
    {
      // Gegner erreichbar: Luftklaue
      if (!Random(ai_iCastChance))
        if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
          if (AI_DoMagic(AFST)) return true;
      // Gegner erreichbar: Magischer Blitz
      if (!Random(ai_iCastChance))
        if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
          if (AI_DoMagic(MCLT)) return true;
      // Gegner sehr nahe: Druckfaust
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 30)
          if (AI_DoMagic(PRFT)) return true;
    }
    if(idStaff==SMER) // Erdzauber
    {
      // Gegner erreichbar: Steinschlag
      if (!Random(ai_iCastChance))
        if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
          if (AI_DoMagic(MARK)) return true;
      // Gegner in der Nähe und Inhaltsobjekt: Objektschleuder
      if (!Random(ai_iCastChance))
        if(Contents()) if( GetMass(Contents())<17 && GetID(Contents()) != EFLN)
          if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
            if (AI_DoMagic(FGOB)) return true;
      // Gegner nahe und viele Objekte: Erdbebenaura
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 120)
	  if(ObjectCount2(Find_Distance(150), Find_Category(16), Find_Not(Find_OCF(OCF_InSolid)))>5) 
            if (AI_DoMagic(AUEQ)) return true;
      // Verbündete Clonks im Umkreis: Platztausch
      var pClonk;
      if (!Random(ai_iCastChance))
      for(pClonk in FindObjects(Find_InRect(-140,-140,280,280), Find_OCF(OCF_CrewMember), Find_NoContainer()))
        if (GetOwner(pClonk) == NO_OWNER) if(ObjectDistance(pClonk) <= 150)
          if (AI_DoMagic(MPXC)) return true;
      // Gegner sehr nahe: Schattenfaust
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 30)
          if (AI_DoMagic(SHFT)) return true;
      // Lust auf Steinschild?
      if (!Random(ai_iCastChance)) if (!Random(ai_iCastChance))
          if (AI_DoMagic(MSSH)) return true;
    }
    if(idStaff==SMFR) // Feuerzauber
    {
      // Gegner erreichbar: Feuerkluster
      if (!Random(ai_iCastChance))
        if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
          if (AI_DoMagic(MFCS)) return true;
      // Gegner sehr nahe: Flammenfaust
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 30)
      ...
      // Gegner in der Nähe und schwächer: Feueraura
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 180)
	  if(GetEnergy()>GetEnergy(ai_pTarget))
            if (AI_DoMagic(AUHE)) return true;
      // Gegner recht nah und nicht brennend: Heißes Blut
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 10)
	        if(!OnFire(ai_pTarget))
            if (AI_DoMagic(MHTB)) return true;
    }
    if(idStaff==SMWT) // Wasserzauber
    {
      // Gegner erreichbar: Schneeball
      if (!Random(ai_iCastChance))
        if(PathFree(GetX(), GetY(), GetX(ai_pTarget), GetY(ai_pTarget)))
          if (AI_DoMagic(MGSW)) return true;
      // Gegner sehr nahe: Eisfaust
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 30)
          if (AI_DoMagic(ICFT)) return true;
      // Gegner nah und nicht eingefroren: Frostwelle
      if (!Random(ai_iCastChance))
        if(Distance(dx,dy) < 50)
	  if(!Frozen(ai_pTarget))
            if (AI_DoMagic(MFWV)) return true;
      // Verbündete Clonks mit Schaden im Umkreis: Gruppenheilung oder Wiederherstellung
      var pClonk;
      if (!Random(ai_iCastChance))
      for(pClonk in FindObjects(Find_InRect(-80,-80,160,160),Find_OCF(OCF_CrewMember),Find_NoContainer()))
        if (GetOwner(pClonk) == NO_OWNER) if(ObjectDistance(pClonk) <= 80)
          if (GetEnergy(pClonk) < GetPhysical("Energy",0,pClonk)/1500)
            {
            if (AI_DoMagic(MHGA)) return true;
            }
    }
  }
...
private func AI_CanCast(id idSpell)
  ...
  if (idSpell)
    {
    if (!GetName(0, idSpell)) return; // Zauber nicht geladen
    if (!CheckMana(idSpell)) return;
    if (ObjectCount(ALCO)) if(!CheckAlchem(idSpell, this)) return;
    // Zauber eingeschränkt?
    if (ai_aSpells) if (GetIndexOf(idSpell, ai_aSpells) < 0) return;
    if (idSpell->~GetSpellClass()) if(idSpell->~GetSpellClass()->~IsStaff())
	    if(idSpell->~GetSpellClass() != idStaff) return;
    }
...
//    if (!Random(3)) if (AI_DoMagic(MGFO)) return true; // Zauber weggefallen
    // Heilzauber
...
private func AI_FindNearestEnemy()
  {
  var aHostile = Find_Not(Find_Owner(-1));
  if(GetOwner()>=0) aHostile = Find_Hostile(GetOwner());
  var obj;
  // Objekte prüfen:
  // Muss leben und im Freien sein, wenn die KI auch im Freien ist...
  for (obj in FindObjects(Find_Distance(ai_iRange), Find_OCF(OCF_Alive), Find_OCF(OCF_CrewMember), Find_Container(Contained()), aHostile, Sort_Distance()))
...
public func AI_IsRanged()
  {
  if (IsMusketEquipped() || IsBowEquipped() || IsCrossbowEquipped()) return true;
...
global func Hostile(int iPlr1, int iPlr2)
  {
  // Hax: Gebäude gehen trotzdem (Wachtürme)
  if (GetCategory() & C4D_Structure) return inherited(iPlr1, iPlr2);
  // Hax für den Fahrstuhlkorb - alle Fahrzeuge nicht, weil einige Zauber intern Fahrzege sind
  if (GetID() == ELEC || GetID() == BRKD || GetID() == CSTE) return inherited(iPlr1, iPlr2);
  if (iPlr1 == NO_OWNER || iPlr2 == NO_OWNER) return iPlr1 != iPlr2;
  return inherited(iPlr1, iPlr2);
  }

// Die Hostile/Allied-Suchen würden normalerweise keine Objekte mit Besitzer -1 annehmen
...
global func GetPlayerName(int iPlr, a,b,c,d)
  {
  // KI-Spieler mit Namen. Ansonsten gehen Revival und SkeletonSummoning nicht
  // Aber nicht für den Zauberturm, weil hack...
  if (iPlr == NO_OWNER) if (GetID() != WTWR) return "AI";
  return inherited(iPlr, a,b,c,d);
  }*/
