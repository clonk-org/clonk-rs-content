...
func Script3() {
  Message("$MsgArtifactWhatever$", FindObject(ASAS)); return(1); }

func Script7() {
 FindObject(TIME) -> SetTime(50);
 var Geheim = ["$MsgCultDiscovered$", "$MsgCultChalice$", "$MsgCultSecret$", "$MsgCultBetrayed$", "$MsgCultLife$", "$MsgCultHideout$", "$MsgCultAttack$"];
 ...
 if(FindObject2(Find_ID(ASAS), Find_InRect(813,549,120,200)) && !GetEffect("Niemand")) { Message("$MsgNobodyPassed$", FindObject(LAWL)); AddEffect("Niemand",0,56); }
 if(FindObject2(Find_ID(ASAS), Find_InRect(752,734,280,50)) && !GetEffect("Geschafft")){ Message("$MsgMadeIt$", FindObject(ASAS)); AddEffect("Geschafft",0,59); }
 if(FindObject2(Find_ID(ASAS), Find_InRect(1205,334,280,50)) && !GetEffect("Aufspreng")){ Message("$MsgExplosives$", FindObject(ASAS)); AddEffect("Aufspreng",0,51); }
 goto(4);
 return(1);
 }
 
 global func Ausruf() {
  Message("$MsgOtherWay$", FindObject(ASAS)); return(1); }
