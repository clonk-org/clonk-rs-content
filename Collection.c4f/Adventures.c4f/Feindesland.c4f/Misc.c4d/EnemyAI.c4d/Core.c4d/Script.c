...
/*for (var i,j=GetPlayerCount(); j; ++i)
    if (GetPlayerName(i))
      {
    //Log("Spieler: %s", GetPlayerName(i));
      if(Hostile(GetOwner(this()), i)) PlayerMessage(i, "<c ff0000>%d", this(), GetEnergy());
      else                             PlayerMessage(i, "<c 0000ff>%d", this(), GetEnergy());
      --j;
  		}*/
  Message("<c ff0000>%d", this(), GetEnergy());
  ...
  if (!AI_IsRanged() || ai_fDoMelee)
    {
    // Zum Gegner laufen
    if (!GetCommand() || (GetCommand() eq "MoveTo" && !Random(8)))
...
private func AI_ExecMovement()
  ...
  if (GetProcedure() eq "WALK")
    {
    if (szCommand eq "MoveTo" || szCommand eq "Attack" || szCommand eq "Follow" || (szCommand eq "Throw" && iTargetX))
      {
  ...
  // Reitend: Bewegungssteuerung ans Tier
    ...
    if (pAnimal = GetActionTarget())
      ...
      if (szCommand eq "MoveTo") 
        ...
        if (szCommand eq "Enter")
          {
          // Enter-Command: Geht nicht. Verwerfen.
          SetCommand(0, "None");
          }
        else if (szCommand eq "Throw")
          {
          // Werfen umsetzen
          ControlThrow();
          SetCommand(0, "None");
  ...
  // Nahkampf:
  // Schon am Kämpfen?
  if (GetProcedure() eq "FIGHT") return(AI_ExecFight());
  ...
  var iPhase;
  if (ai_pTarget) if (GetAction() eq "Walk")
    {
    iPhase = GetPhase();
...
protected func AI_WaterJumpOut(object pThis, int iJumpDir)
  ...
  if (GetProcedure() ne "SWIM") return();
  if (iJumpDir>0)
    {
    SetComDir(COMD_Right);
    SetDir(DIR_Right);
...
private func AI_CanConjure(id idWonder)
  ...
  return (GetProcedure() eq "WALK" || GetProcedure() eq "ATTACH");
  }
...
protected func ContactLeft()
  ...
  if (IsAnimal()) if (GetCommand() eq "MoveTo") if (GetCommand(0, 2)) if (GetCommand(0, 2) < GetX())
    {
