...
protected func Construction()
{
  ...
  AddEffect("AdjustHangPos", this(), 1, 1, this());
}

public func GetStaLength()
{
  return (GetCon() * GetDefCoreVal("Height") / 100);
}

public func FxAdjustHangPosTimer()
{
  // Effekt entfernen
  RemoveEffect("AdjustHangPos", this());
  // Y-Position anpassen (Wachstumsverschiebung der Engine ausgleichen)
  SetPosition(GetX(), GetY() - 4 + GetStaLength());   
}