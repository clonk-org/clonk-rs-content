...
protected func DoJump() {
  ...
  // Aktivität setzen
  Jump(target);  SetSpeed(Sin(iAngle, 78), -(Cos(iAngle, 78)*4/3), target);
...
protected func CastError(pstring, caster, silent) {
  // Meldung ausgeben
  if (!silent) Message(pstring, caster, GetName(caster));
  // Sound
