...
private func DoPlrLaunch()
{
 ...
 var x; if (GetPlayerTeam(iSpieler) == 1) x = 500; else x = 3030;
 while (Contents()) Exit(Contents(), x-GetX(), 550-GetY());
...
public func SetCastleColor(iPlayer, iPosx, iPosy, iWidth, iHeight)
{
 ...
 for (var pObj in FindObjects(Find_InRect(iPosx-GetX(), iPosy-GetY(), iWidth, iHeight),Find_ID(IDOL))) pObj->Initialize();
 for (var pObj in FindObjects(Find_InRect(iPosx-GetX(), iPosy-GetY(), iWidth, iHeight),Find_ID(GIDL))) pObj->Initialize();
