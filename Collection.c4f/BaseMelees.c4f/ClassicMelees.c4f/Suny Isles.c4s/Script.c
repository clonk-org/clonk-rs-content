...
protected func VegiSetUp()
{
  ...
  Message("$Processing$");
  var VegisOnGround =[BONE,SHEL,SKUL,SWD2,SWD1,GIDL,IDOL,CRYS];			// IDs der möglichen Vegitation (doppelt so viele Lianen wie Stalaktiten)
  var VegisFactor = [100,20,50,500,500,5,10,5];					// Anzahl für Häufigkeit
  ...
  for ( var k=0; k<=LandscapeHeight(); k+=LandscapeHeight()/10 )		// Obere Schleife: Teilt die Lanschaftshöhe in 10 gleich hohe Zeilen
	for (var i=0; i<LandscapeWidth()/150; i++)				// Mittlere Schleife: Platziert in jeder Zeile LandscapeWidth()/50 Objekte
		{
