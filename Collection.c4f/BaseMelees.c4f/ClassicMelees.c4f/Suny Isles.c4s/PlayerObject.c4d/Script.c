...
private func DoPlrLaunch()
{
 ...
 // Alle Spieler durchgehen, falls dieser der Erste in seinem Team ist, Segelboot erschaffen.
 var i = GetPlayerCount();
 while (i--)
  if (GetPlayerByIndex(i) != iSpieler && GetPlayerTeam(GetPlayerByIndex(i)) == GetPlayerTeam(iSpieler))
   return(RemoveObject());

 if (GetPlayerTeam(iSpieler) == 2) CreateObject (SLBT,1360,320,-1);
  else CreateObject (SLBT,-1360,320,-1);
 return(RemoveObject());
}
