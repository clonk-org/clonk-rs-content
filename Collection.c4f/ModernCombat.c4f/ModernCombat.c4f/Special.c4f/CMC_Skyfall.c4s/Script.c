...
public func RecommendedGoals()	{return [GLMS];}	//Spielzielempfehlung

static const FKDT_SuicideTime = 4;		//Verkürzte Wartezeit

static const SPAWNSYS_Allies	= 100;
static const SPAWNSYS_Enemies	= -100;
static const SPAWNSYS_Traps	= -100;
...
func Initialize()
{
  ...
  if(C4X_VerBuild < 331) {SetNextMission("ModernCombat.c4f\\Special.c4f\\CMC_Skyfall.c4s", "$RepeatButton$", "$RepeatButtonDesc$");}
  return 1;
}
...
func CreateDecoration()
{
  ...
  rail = CreateObject(RAI1, 528, 1210, -1);
  rail->SetCategory(C4D_StaticBack);
  rail->SetRail([1,3,1,3,1]);
  for(var i = 0; i <= 6; i++)
    rail->SetClrModulation(RGB(200, 200, 200), this, i);
  ...
  for(var i = 0; i <= 16; i++)
    rail->SetClrModulation(RGB(200, 200, 200), this, i);
  rail = CreateObject(RAI1, 4008, 1210, -1);
  rail->SetCategory(C4D_StaticBack);
  rail->SetRail([1,3,1,3,1]);
  for(var i = 0; i <= 6; i++)
    rail->SetClrModulation(RGB(200, 200, 200), this, i);
  ...
  //Pumpen
  CreateObject(PMP2, 560, 1218, -1)->SetClrModulation(RGB(125,125,125));
  CreateObject(PMP2, 4040, 1218, -1)->SetClrModulation(RGB(125,125,125));
...
public func OnClassSelection(object pClonk, int iClass)
{
  if(!FindObject(GOCC))
  {
    CreateHelicopterSpawner(pClonk);
  }
}
...
public func ChooserFinished()
{
  ...
  CreateObject(SGNP, 670, 1380, -1)->Set("Danger",0,1);
  CreateObject(SGNP, 1610, 1380, -1)->Set("Danger",0,1);
  CreateObject(SGNP, 2980, 1380, -1)->Set("Danger",0,1);
  CreateObject(SGNP, 3865, 1380, -1)->Set("Danger",0,1);
  ...
  if(FindObject(GOCC))
  {
    ...
    if(aTeams[1] == true)
    {
      aSelfDefense[0] = CreateObject(SEGU, 560, 1210, -1);
    ...
    if(aTeams[2] == true)
    {
      aSelfDefense[1] = CreateObject(SEGU, 4040, 1210, -1);
    ...
    //Hinweisschilder
    CreateObject(SGNP, 495, 895, -1)->Set("Turret",0,1);
    CreateObject(SGNP, 4085, 895, -1)->Set("Turret",0,1);
  }
...
public func RelaunchPosition(& iX, & iY, int iTeam)
{
  ...
  if(FindObject(GLMS))
  {
    return [[350, 760], [540, 950], [400, 1160], [710, 770], [4250, 760], [4060, 950], [4200, 1160], [3890, 770], [1160, 170], [1150, 500], [1630, 405], [1440, 670], [3450, 170], [3460, 500], [2960, 405], [3160, 670]];
    return 1;
  }
}
