...
func Initialize()
{
  ...
  if(C4X_VerBuild < 331) {SetNextMission("ModernCombat.c4f\\Classic.c4f\\CMC_Yuki.c4s", "$RepeatButton$", "$RepeatButtonDesc$");}
  return 1;
}
...
func CreateEquipment()
{
  ...
  CreateObject(AMCT, 935, 660, -1)->Set(ABOX);
  CreateObject(AMCT, 1975, 660, -1)->Set(ABOX);

  //Versorgungskiste (Gewehrgranaten)
  CreateObject(AMCT, 1500, 430, -1)->Set(GBOX);

  //Versorgungskisten (Dragnin)
  CreateObject(AMCT, 1070, 760, -1)->Set(DGNN);
  CreateObject(AMCT, 1895, 830, -1)->Set(DGNN);
...
func CreateDecoration()
{
  ...
  //Leitkegel
  CreateObject(TCCN, 795, 720, -1);
  CreateObject(TCCN, 925, 750, -1);
  CreateObject(TCCN, 1325, 770, -1);
  CreateObject(TCCN, 1395, 750, -1);
  ...
  //Hinweisschilder
  CreateObject(WLSN, 830, 735, -1)->Set("Heights","Wood");
  CreateObject(WLSN, 1050, 520, -1)->Set("Explosives","Red");
  CreateObject(WLSN, 1360, 765, -1)->Set("Heights","Wood");
  CreateObject(WLSN, 1785, 800, -1)->Set("Heights","Wood");
...
public func ChooserFinished()
{
  ...
  if(FindObject(GHTF))
  {
    ...
    if(!FindObject(NOAM))
    {
      ...
      CreateObject(AMCT, 1510, 660, -1)->Set(ABOX);
    }
  }
