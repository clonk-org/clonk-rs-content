...
func Initialize()
{
  ...
  if(C4X_VerBuild < 331) {SetNextMission("ModernCombat.c4f\\CMC_PortArcford.c4s", "$RepeatButton$", "$RepeatButtonDesc$");}
  return 1;
}
...
func CreateEquipment()
{
  ...
  CreateObject(AMCT, 1170, 550, -1)->Set(ABOX);
  CreateObject(AMCT, 3020, 690, -1)->Set(ABOX);
  CreateObject(AMCT, 5300, 560, -1)->Set(ABOX);

  //Versorgungskisten (Gewehrgranaten)
  CreateObject(AMCT, 1310, 530, -1)->Set(GBOX);
  CreateObject(AMCT, 2970, 700, -1)->Set(GBOX);
  CreateObject(AMCT, 5330, 560, -1)->Set(GBOX);
...
public func ChooserFinished()
{
  ...
  if(FindObject(GOCC))
  {
    ...
    CreateObject(AMCT, 3380, 630, -1)->Set(PGRN);
    ...
    if(!FindObject(NOHC))
    {
      ...
      CreateObject(SGNP, 990, 500, -1)->Set("Helicopter");
      CreateObject(SGNP, 5035, 480, -1)->Set("Helicopter",1);
    }
  }
  ...
  if(FindObject(GASS))
  {
    ...
    CreateObject(AMCT, 1435, 650, -1)->Set(DGNN);

    //Versorgungskiste (Sensorbälle)
    CreateObject(AMCT, 4380, 690, -1)->Set(SRBL);

    //Versorgungskiste (Sprengfallentaschen)
    CreateObject(AMCT, 5480, 660, -1)->Set(BTBG);
  ...
  if(FindObject(GMNR))
  {
    ...
    CreateObject(BRDR, 4210, 0, -1)->Set(1,0,0,0,0,1);
