...
protected func Timer()
{
  ...
  var Color;
  if(!(Color = Contents()->~GetSpawnColor()))
  	Color = RGBa(210, 210, 255, 100);
...
public func RejectEntrance(object pClonk)
{
  ...
  if(CheckCollect(GetOwner(pClonk), pClonk))
  {
    ...
    // Kann der Clonk einsammeln?
    if(pObj->~IsAmmoPacket()) {
      if(!(pObj->MayTransfer(pClonk)) || NoAmmo())
      {
      	RemoveObject(pObj);
        return(1);
      }
    ...
    else
    {
      ...
      {
        var ammopack = pObj->~GetAmmoPackID();
        if(ammopack)
        {
        	var pack = CreateObject(ammopack);
        	pack->Activate(pClonk);
        	if(pack)
        		pack->RemoveObject();
        }
      }
