...
func Initialize() {
  ...
  if(!FindObject(CROB)) CreateObject(CROB);
...
global func FillTheWorldWithLife() {
  // Algen, Anker, Seegras, Muscheln, Palmen, Büsche, Haie, Fische, Knochen, Kaputte dinger
    ...
    PlaceInSea(SHRK, 3);    
    // Palmen und so
    PlaceOnLand(PLM2, 30,0,0,0,0,0, 1);
    PlaceOnLand(PLM1, 26,0,0,0,0,0, 1);  
    PlaceOnLand(BUSH, 24,0,0,0,0,0, 1);
    PlaceOnLand(PLM2, 40,0,0,0,0,0, 0);
    PlaceOnLand(PLM1, 26,0,0,0,0,0, 0);  
    PlaceOnLand(BUSH, 24,0,0,0,0,0, 0);
    // bla und so
    PlaceOnLand(NOVO, 1);
    PlaceOnLand(HTB1, 1); 
    PlaceOnLand(GDLS, 3);     
