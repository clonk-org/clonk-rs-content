...
#include PPT1 // Burgteil - Treppenhausfunktion
...
public func ContextGateLeft(object pClonk)
{
  ...
  AppendCommand(pClonk, "Call", this, 0,0,0,0, "ControlLeft");  
  return 1; 
}
...
public func ContextGateRight(object pClonk)
{
  [$TxtRightgate$|Image=GATR|Condition=FindGateRight]
  SetCommand(pClonk, "Grab", this);
  AppendCommand(pClonk, "Call", this, 0,0,0,0, "ControlRight");  
  return 1; 
}
...
public func ContextBridgeUp(object pClonk)
{
  ...
  SetCommand(pClonk, "Grab", this);
  AppendCommand(pClonk, "Call", this, 0,0,0,0, "ControlUp");  
  return 1; 
}
...
public func ContextBridgeDown(object pClonk)
{
  ...
  SetCommand(pClonk, "Grab", this);
  AppendCommand(pClonk, "Call", this, 0,0,0,0, "ControlDownSingle");  
  return 1; 
}
...
protected func Initialize()
{
  ...
  var pWall;
  pWall = CreateObject(PPW3, 0, +30, GetOwner());
  // An Hintergrund-Wand ausrichten
  SetPosition(pWall->GetX(), pWall->GetY() + 7);
  // Brustwehren erzeugen
  CreateObject(PPB1, -21, -27, GetOwner())->AttachTo(pWall, DIR_Left);
  CreateObject(PPB1, +22, -27, GetOwner())->AttachTo(pWall, DIR_Right);
  // Niedergang erzeugen
