...
{
//  if((GetAction()S="Elevator"||GetAction()S="EnterElev")) { SetAction("ExitElev"); return(0); }
  // Während der Fahrt
  var iPos = -24;
  // Während der Wende
  if (GetAction() eq "Turn") iPos = 20 - 2 * GetPhase();
  // Während der Wende
  if (GetAction() eq "EnterElev") iPos =-20 + 2 * GetPhase();
  // Während der Wende
  if (GetAction() eq "ExitElev") iPos = 0 - 2 * GetPhase();
...
public func FitsInElevator() { return(1); }

public func EnterElevator()
{
  if(GetAction()S="EnterElev") return(1);
  if(GetAction()S="Elevator") return(1);
  SetAction("EnterElev");
  return(1);
}

protected func StartElevator()
{
  if(!GetElev()) return(LeaveElevator());
  SetDir(1);
}

public func LeaveElevator()
{
  if(GetAction()S="Elevator") SetAction("ExitElev");
}
...
// Keine Brückensegmente aufnehmen
public func RejectCollect(idObj, pObj)
{
  if(idObj==BRDG) return(1); 
