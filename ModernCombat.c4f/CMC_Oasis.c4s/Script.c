...
func Initialize()
{
  ...
  if(C4X_VerBuild < 331) {SetNextMission("ModernCombat.c4f\\CMC_Oasis.c4s", "$RepeatButton$", "$RepeatButtonDesc$");}
  return 1;
}
...
func CreateInterior()
{
  ...
  //Sandsackbarrieren
  CreateObject(SBBA, 2790, 600, -1)->Set(1,1);
  CreateObject(SBBA, 4041, 600, -1)->Set(0,1);
...
func CreateEquipment()
{
  ...
  CreateObject(AMCT, 1390, 460, -1)->Set(ABOX);
  CreateObject(AMCT, 3450, 530, -1)->Set(ABOX);
  CreateObject(AMCT, 5440, 460, -1)->Set(ABOX);

  //Versorgungskiste (Gewehrgranaten)
  CreateObject(AMCT, 3380, 530, -1)->Set(GBOX);

  //Versorgungskisten (APW)
  CreateObject(AMCT, 1980, 690, -1)->Set(ATWN);
  CreateObject(AMCT, 4850, 690, -1)->Set(ATWN);
...
public func ChooserFinished()
{
  ...
  if(!FindObject(NOHC))
  {
    ...
    CreateObject(SGNP, 940, 600, -1)->Set("Helicopter",1);
