...
func Initialize()
{
  ...
  if(C4X_VerBuild < 331) {SetNextMission("ModernCombat.c4f\\Classic.c4f\\CMC_MacrossFrontier.c4s", "$RepeatButton$", "$RepeatButtonDesc$");}
  return 1;
}
...
func CreateEquipment()
{
  ...
  CreateObject(AMCT, 275, 550, -1)->Set(ABOX);
  CreateObject(AMCT, 1675, 460, -1)->Set(ABOX);
...
func CreateDecoration()
{
  ...
  //Leitkegel
  CreateObject(TCCN, 610, 650, -1);
  CreateObject(TCCN, 640, 660, -1);
  CreateObject(TCCN, 825, 540, -1);
  CreateObject(TCCN, 1125, 460, -1);
  CreateObject(TCCN, 1310, 580, -1);
  CreateObject(TCCN, 1340, 570, -1);
  ...
  //Hinweisschilder
  CreateObject(WLSN, 640, 650, -1)->Set("Explosives","Red");
  CreateObject(WLSN, 1005, 635, -1)->Set("Extinguisher","Red");
  CreateObject(WLSN, 1325, 570, -1)->Set("Explosives","Red");
  ...
  CreateObject(FLGH, 815, 350, -1)->Set(1,1,0,40);
...
public func ChooserFinished()
{
  ...
  //Starttitel und sliste zusammenstellen
  ...
  if(FindObject(GHTF))
  {
    ...
    if(!FindObject(NOAM))
    {
      ...
      CreateObject(AMCT, 960, 640, -1)->Set(ABOX);
    }
  }
