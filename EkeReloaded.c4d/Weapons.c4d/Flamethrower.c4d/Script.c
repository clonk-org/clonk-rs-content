...
{  
  if (!WildcardMatch(GetAction(clonk), "Flamethrower*")) return(1);
...
func Shooting()
{
  ...
  var b1 = !WildcardMatch(GetAction(clonk), "Flamethrower*");
...
func PilotLight()
{
  ...
  if (!WildcardMatch(GetAction(clonk), "Flamethrower*")) return(0);

  var x = 20 * dir - 10 + Random(2);
  var y = RandomX(-3, -1);

  CreateParticle("PSpark", x, y, 0, -3, 20, RGBa(255, Random(2) * 100, 0, 120), clonk);
  return(1);
}
...
func CreateFire(object clonk, int dir)
{
  ...
  var fire = CreateObject(FI5B, x, y, NO_OWNER);
